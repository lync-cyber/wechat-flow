...
| I-4 | 创建 CLAUDE.md | 按 Update Template 生成，文档状态全 `未开始`，当前阶段按模式（requirements/planning/brief） | D | 初始阶段与模式是否一致 |
...
| id | 子步 | 期望行为 | 观察重点 |
|----|------|---------|---------|
| C-5a | 文档头 + CLAUDE.md 状态更新 | status: draft→approved，阶段字段同步 | 文档头与 CLAUDE.md 是否一致 |
