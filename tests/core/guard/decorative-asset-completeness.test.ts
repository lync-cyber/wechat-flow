...
// ---------------------------------------------------------------------------
// AC-009: decorative-asset-completeness
// ---------------------------------------------------------------------------

describe("decorative-asset-completeness: positive — no assets declared", () => {
  it("does not produce a failure when assets is an empty object", () => {
    const theme = makeTheme({});
    const result = validateThemeGuard(theme);
    const failure = result.failures.find((f) => f.dimension === "decorative-asset-completeness");
    expect(failure).toBeUndefined();
  });

  it("does not produce a failure when assets is undefined", () => {
...
describe("decorative-asset-completeness: positive — assets declared with non-empty values", () => {
  it("does not produce a failure when all declared asset values are non-empty strings", () => {
    const theme = makeTheme({
      "divider-ornament": "<svg>...</svg>",
      "header-decoration": "<svg><!-- ornament --></svg>",
    });
    const result = validateThemeGuard(theme);
    const failure = result.failures.find((f) => f.dimension === "decorative-asset-completeness");
    expect(failure).toBeUndefined();
...
describe("decorative-asset-completeness: negative — declared asset with empty string value", () => {
  it("produces error when an asset entry has an empty string value", () => {
    const theme = makeTheme({ foo: "" });
    const result = validateThemeGuard(theme);
    const failure = result.failures.find((f) => f.dimension === "decorative-asset-completeness");
  ...
  it("error message contains the key with the empty value", () => {
    const theme = makeTheme({ foo: "" });
    const result = validateThemeGuard(theme);
    const failure = result.failures.find((f) => f.dimension === "decorative-asset-completeness");
    expect(failure?.message).toContain("foo");
  });

  it("produces error for each key with empty value when multiple are empty", () => {
    const theme = makeTheme({ "asset-a": "", "asset-b": "", "valid-asset": "<svg/>" });
    const result = validateThemeGuard(theme);
    const failure = result.failures.find((f) => f.dimension === "decorative-asset-completeness");
    expect(failure).toBeDefined();
    expect(failure?.message).toContain("asset-a");
    expect(failure?.message).toContain("asset-b");
    // valid asset should not be mentioned
    expect(failure?.message).not.toContain("valid-asset");
  });
});
