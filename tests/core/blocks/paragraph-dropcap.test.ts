...
// AC-001: paragraph.variants 新增 dropcap，渲染后首字符独立 <span>，font-size 2.2em、display inline-block、vertical-align top
describe("AC-001: paragraph dropcap 变体渲染首字符独立 span", () => {
  ...
  it("渲染后 HTML 含首字符独立 <span>", async () => {
    ...
    expect(result.html).toMatch(/<span style="[^"]*">首<\/span>/);
  });
  ...
  it("首字符装饰元素计算 font-size 换算值等于 2.2em", async () => {
    ...
    const match = result.html.match(/<span style="([^"]*)">首<\/span>/);
    expect(match).not.toBeNull();
    expect(match?.[1]).toContain("font-size: 2.2em");
  });

  it("首字符装饰元素计算 display = inline-block", async () => {
    const result = await renderMarkdown(":::paragraph{.dropcap}\n首字后面的正文\n:::", {
      themeId: "default",
    });
    const match = result.html.match(/<span style="([^"]*)">首<\/span>/);
    expect(match?.[1]).toContain("display: inline-block");
  });
...
describe("AC-004: paragraph 默认渲染不受 dropcap variant 影响（回归验证）", () => {
  it("普通段落（无 container directive）渲染为 <p> 标签，不含 dropcap span", async () => {
  ...
  it("普通段落渲染不含独立首字符 <span>", async () => {
    const result = await renderMarkdown("首字后面的正文", { themeId: "default" });
    expect(result.html).not.toMatch(/<span style="[^"]*">首<\/span>/);
  });
});
