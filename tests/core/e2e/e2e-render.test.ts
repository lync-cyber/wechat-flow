...
describe("T-057 E2E render fixtures (Markdown → final inline-styled HTML)", () => {
  for (const name of fixtureNames) {
    it(`${name}: deterministic byte-exact snapshot, sanitized, paste-safe`, async () => {
      ...
      // AC-004: the render product is a fixed point of the wechat platform filter —
      // wechatAdapter.inspect finds nothing further to strip (theme inline styles survive).
      expect(wechatAdapter.inspect(html).changes).toEqual([]);
...
describe("T-057 AC-004: whitelisted inline style survives wechat paste filter", () => {
  it("a theme-styled fixture keeps its inline style attribute under wechatAdapter.inspect", async () => {
    const input = readFileSync(join(FIXTURES_DIR, "01-basic-heading-paragraph.md"), "utf8");
    const { html } = await renderMarkdown(input, { themeId: "default" });
    expect(html).toContain('style="');
    expect(wechatAdapter.inspect(html).patchedHtml).toContain('style="');
  });
});
