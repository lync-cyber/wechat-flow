...
- 当前阶段: development
- 上次完成: **T-036 done** — MCP server stdio transport + 鉴权骨架（feature/mcp-server 分支，未推送）。light-dispatch TDD：装 `@modelcontextprotocol/sdk@1.29.0`（app deps + root devDeps）、`createServer` 注册 24 Tool（消费 contracts `ALL_TOOL_SCHEMAS` 占位，真实实现待 T-037/038/039/122）、StdioServerTransport、API key `sha256`+`timingSafeEqual` 验证、scope-guard fail-closed（user 放行 / admin→`E_PERMISSION_DENIED` / 无 key→`E_AUTH_REQUIRED`）。code-review **r1 needs_revision**（HIGH R-001：`registerAllTools` 硬编码 null keyRecord → 鉴权端到端失效）→ revision 把 keyRecord 经 `createServer(apiKeyStore + rawApiKey from env)` 贯通 → **r2 approved**。15 tests（含 InMemoryTransport+`callTool` 端到端鉴权 3 例），typecheck/biome/全量 679 全绿。前序：dev-plan s4 amendment（T-030/031 落点 composers/→use-cases/，commit 309037f，feature/mcp-server 基于 fix 分支故随行）；framework 0.13.0；输出能力 T-030/031（PR #8）；L3 cascade（PR #6）。运行学习：①接线类 AC 须行为级测试 ②standard 跨包卡子代理易 truncation→主线程接管 ③不轻信自报告、主线程取地面真值复核（本切片 implementer 两度自报 typecheck clean，实为漏跑 `tsc -p tests/tsconfig.json`(TS2307) + stale IDE 诊断，均主线程复核纠正）④白名单 SSOT=packages/core/src/registry/css-property-whitelist.ts ⑤`cataforge deploy --rebuild` 会删 CLAUDE.md 致 §项目状态 静默 wipe，刷产物用普通 deploy ⑥app 私有第三方依赖被集中式 `tests/` 引用时须同时提到 root devDeps（与 zod/vue 同），否则 root `tests/tsconfig.json` tsc 解析不到（TS2307）；勿用 vitest.config alias 半截 hack（只修 runtime 不修 tsc）。
- 下一步行动: T-036 已 commit feature/mcp-server。下一步**并行** T-037（render_markdown/lint_markdown/get_ruleset_version）+ T-038（list/describe themes/blocks/marks）+ T-122（register_variant）—— 三者仅依赖 T-036、互不依赖可并行。关键校正：T-037 dev-plan 写「连接 composeRender」，但 composeRender 在 apps/editor（应用层），mcp-server 是另一 app 不能跨 import use case → 应直接接 `@wechat-flow/core` 的 `renderMarkdown`（framework-agnostic，core 已导出）。后续切片：relay 后端(T-032..035/091)、编辑器 UI(T-040/041/042/093)、主题 template T-092、DESIGN(T-102/105/106)、VALIDATION T-111。
...
- 当前Sprint: Sprint 4 进行中 — MCP server 切片 **T-036 done**（feature/mcp-server，本地未推送，含 dev-plan amendment 两 commit）；输出能力切片 **PR #8 已合 main**；L3 cascade 切片已合 main（PR #6）。Sprint 0-3 已关闭合 main（PR #1/#2/#6/#7 均已合）。
