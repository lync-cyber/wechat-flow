...
- 当前阶段: development
- 上次完成: **T-131 前端 vs Penpot 一致性门禁（[PR #66](https://github.com/lync-cyber/wechat-flow/pull/66)）** —— verdict=**conditional_release**（非 approved）。客观门禁全绿：`pnpm design:token-diff` 0 mismatch exit0（83 token）、frames 23 UC+5 P（`git ls-files docs/design/`=32 文件）、[`overlay-report.html`](docs/design/reports/overlay-report.html) `23/23`+`5/5`、design-sync 11 单测过。视觉自查 14 节一致（页 P-001/P-003/P-004 + 结构组件 UC-001/002/004/005/006/007/013/022/023），用户 sign-off（EVENT-LOG `user_decision` ref=T-131）。**Issue A 当场闭环**：全 live 文件清查 **0 处真实 C-NNN**（Design System 页 `AC-003/004` 是验收编号，被 `(?<!U)C-\d{3}` 误配的假阳性），committed 帧中 **4 张**（UC-001/UC-002/UC-013/P-001）系 T-130 改名前 stale 导出 → 重导覆盖 UC-NNN 化验证通过；其余 15 非-S6 帧重导**字节全等**（本就干净）、S6 9 帧 scale-2 原生 UC-NNN 未动。**残差 blocking_conditions（随 sprint-review 收口，非空前 Sprint 6 不 DONE）**：[B1] P-002 文档列表前端未渲染（编辑器文档 Tab 仅空标签，缺新建/列表/empty 态）owner=developer；[B2] P-005 移动预览正文空（demo fixture 未 seed，chrome 一致，LOW）owner=developer；[C] 14 交互触发组件（palette/menu/toast/modal/drawer/popover/dialog）static overlay 未捕获，设计板已 T-130 sign-off、实现各构建卡已验证 owner=developer。**跨界经验修订**：`export_shape` 的 `filePath` 写 Penpot MCP 容器 fs → `docker cp penpot-docker-penpot-mcp-1:/tmp/... "$(pwd -W)/..."`（`MSYS_NO_PATHCONV=1`）拉宿主，比 base64 灌上下文干净；`export_shape` **不受当前页限制**但默认 **scale=1**（需 scale=2 时走 plugin `shape.export({scale})`）；cross-page `findShapes()`/`characters` **读**可靠，仅 mutate/plugin-export 受当前页限（记忆 [[penpot-plugin-gotchas]]）。前批 **T-130 设计一致性工具链 + 8 UC 补画 + C-→UC 命名统一（[PR #65](https://github.com/lync-cyber/wechat-flow/pull/65)）**、**T-104 深色主题 token（[PR #64](https://github.com/lync-cyber/wechat-flow/pull/64)）**、T-128（[PR #63](https://github.com/lync-cyber/wechat-flow/pull/63)）；各卡细节见 EVENT-LOG 与 PR 记录。
- 下一步行动: 链条 `T-104✓→T-130✓→T-131✓(conditional_release)→T-113`。**T-131 已合，客观门禁全绿 + Issue A 闭环；遗 3 条 blocking_conditions（B1/B2/C，见上次完成）未收口→ `conditional_release.blocking_conditions` 非空，Sprint 6 未 DONE**。**下一步 = sprint-review 逐条定夺 B1/B2/C**（accept-with-rationale / 新卡 / backlog）→ 清空 blocking_conditions → **T-113**（Sprint 6 验证门，14 项）→ sprint-review 收尾。倾向：B2=fixture 未 seed、C=overlay 方法局限且实现各卡已验证 → 两者 **accept-with-rationale**；**B1（P-002 文档列表前端未 wire-in）需用户定**：新前端卡 vs backlog（deferred）。节奏：单卡一 PR、串行合并（§项目状态 单写区）。设计轨道 T-095..T-106 全 sign-off（含 T-104）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟（配套 Linux 视觉基线重播）；② T-085 skill bundle description 自动 optimizer 未跑（无头 `claude -p` 401）；③ **S6 deliverable/落点漂移聚合**（sprint-review 汇总 drift-rate，勿逐卡回改；root_cause=upstream-caused）；根因已打包上游反馈 `docs/feedback/fb-dev-plan-path-drift-writeback-20260701.md`。**T-113 forward-drift**：AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
