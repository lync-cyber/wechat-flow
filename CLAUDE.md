...
- 上次完成: **CataForge 0.14.0 → 0.15.0 同步（PR #99）+ BC-14 收敛（PR #98）**：`cataforge bootstrap` 刷 scaffold + 重渲 IDE 产物，**doctor 全绿**（修复 context.mode hybrid 枚举失效 FAIL）；context.mode `hybrid`→`graph` 自动迁移（store 非空+有快照，seed 不触发，**docs/*.md 零改写**）；penpot 三 skill→penpot-bridge、新增 feature-walkthrough/project-visualization；upgrade.state 簿记 0.15.0/CataForge@46d025b；业务代码未动。BC-14=UC-014 JobProgressBar Penpot 帧经检查已完整重绘、5 状态 token 零失配、用户 sign-off（设计侧 open 4→3；先检查后动手避免覆盖已正确成果）。
- 下一步行动: **项目 completed，剩余均为用户侧决策**：① **release go/no-go**（包版本 0.0.0 核对 / mcp-server 发布面 private 翻转→发布集 13→14 / npm-publish reviewers / 真实公众号粘贴回归 / Docker bring-up / CVE 阈值——细节见 §待办 + deploy-spec §9）② **CataForge 0.15.0 升级已完成**（PR #99，见 上次完成）③ **设计一致性 backlog**：dev 侧 BC-1..BC-11+BC-14 全收敛，余 BC-12/13/15（ui-designer）+ BC-4 派生 2 feature 缺口见 §待办 ④ **amendment 批现可推进**（前置已解）：arch 措辞修订 5 项 + BC-12/13/15 Penpot 对齐合批（graph 模式经 context write/finalize 落图）。
...
- 待办(deferred)（仅列 open 项；已解决项见 git/PR 历史）:
  - **设计一致性收敛 backlog（T-131 AC-004 二轮 sign-off，verdict=conditional_release；逐条见 `SPRINT-REVIEW-s6-r3.md`）**: 15 条 blocking_conditions disposition 至此（不回退 completed）。**已收敛 12 条**：BC-1..11 + BC-14（PR #94–98，细节见各 PR）。**余 3 条 open（owner=ui-designer，随 amendment 批同步 Penpot）**：BC-12 UC-020 派生分组、BC-13 UC-022 selected 满铺（均裁决前端权威/Penpot 追平，并入 BC-15）、BC-15 设计侧对齐批（kbd 徽章/视口切换器/主题卡/条色/文案等，明细见 r3）。**BC-4 派生 2 feature 缺口**：可读性评级算法待 product spec、夜间风险 nightRiskIssues render pipeline 填充。判一致 4 节 + LOW 散点见 r3。
  - **arch 措辞修订登记（owner=architect；framework-update 前置已解，本批现可执行，与 BC-12/13/15 Penpot 对齐合批）**: ① M-003 fixture 目录措辞对齐实落地 `packages/ruleset/src/rules/builtin/{rule-id}/` ② F-011 PatchBundle 对齐声明式 DSL（`docs/research/tech-eval-patch-dsl.md` rn-007）③ API-032 refreshUntil=expiresAt（续期窗口起点 exp−60s）④ ui-spec UC-022 对齐双按钮功能超集（换主题/载入模板，Penpot 补双按钮布局）⑤ 设计稿模板示意与实现模板集（listicle/case-study）语义对齐。
