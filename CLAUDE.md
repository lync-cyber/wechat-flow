...
- 当前阶段: development（Sprint 7 收尾：变体缺口方案批 A/B 已合 main，批 C 收口 PR #133 待合，剩 T-212 用户走查）
- 上次完成: **变体缺口方案批 C 收口 PR #133（2026-07-16，待合，分支 feature/variant-gap-batch-c）**——批 B（#130）+ 批 C 前置缺陷 A/C（#131）+ decorate-utils 提取（#132）均已合 main（0171bf5）。#133 内容：T-206 gradient DELETE（用户 sign-off，归零对账实证唯一剩余项，删后空集）· T-209 registerBlock collect-only→always-throw E_VARIANT_NO_IMPL、移 mode 开关（setVariantGuardMode/variantGuardMode）+ candidates 脚手架、getUnimplementedVariants 重实现为 listBlocks() 纯扫描（13 批 B 验收测试续消费）· T-210 差分守卫 WARN→RED（移 console.warn、真实注册表 finding 断言空集）· **gallery 变体目录整体重裁 6→3**（default 单列全宽 / duo 2列 / triptych 3列，删 grid/masonry/carousel 冗余别名，DESIGN-REVIEW-gallery-r2 用户 sign-off；根因 default 经 decorate 折算自渲染为 duo 态致 duo/grid 与 default 塌缩字节同构，重裁使 default 独立形态→finding 归零无需 allowlist；实为修复潜伏 code↔baseline 漂移）· T-211 部分（assertVariantFloor 120→131 实测值）。夹具：no-op 探针块补 decorate 过 always-throw 注册守卫；ui-spec §10.9 amend。四门禁全绿（vitest 4666 / cross-runtime SHA 不变 / biome / typecheck）；视觉删 40 孤儿快照（gradient/grid/masonry/carousel ×2平台），visual-update 全矩阵 --update-snapshots 735 passed 零漂移，PR CI visual-core/sampled 全绿。
  - 历史批次见 git/PR 历史与 EVENT-LOG，不在此累积。
- 下一步行动:
  - ① **PR #133 合并**：CI 全绿（visual-core/sampled/cross-runtime/quality-gate/guards/bench pass），用户手合（gh classifier 拦自合并）。批 C 三类前置已全部消解：(a) gradient 用户 DELETE（本 PR）· (b) 谓词④ divider external-pipeline（#131）· (c) 差分守卫内容感知 fixture（#131 list/code + 本 PR gallery 重裁）。
  - ② **T-212 用户走查验证（owner=user，批 C 最后一卡）**：编辑器 InsertDrawer/变体选择器抽样核对本批新增具名变体可选中且预览符 triage 裁定意图；结构性变体（steps/quote/pull-quote/qa/dialog/tip-grid/definition-list/timeline 等）DOM 走查；确认已删除项（T-208 十项 + gradient + gallery grid/masonry/carousel）不再出现于任何用户可选界面；audio/video default 不受 T-207 移除影响；产出 `WALKTHROUGH-variant-gap-remediation-{date}-r1` + 写 event=user_decision。
  ...
  - 独立分流：decorate-utils.ts 提取已随 #132 合并（原 spawn_task task_6198f478 已落地）。
