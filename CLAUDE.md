...
- 当前阶段: development
- 上次完成: **T-130 设计一致性工具链 + 8 UC 补画 + Penpot 命名统一（[PR #65](https://github.com/lync-cyber/wechat-flow/pull/65)）** —— 全 7 AC 客观达成。①4 个 design-sync 脚本（`token-diff` 83 token 0 mismatch exit0 / `export-penpot-tokens` 校验≥60 / `export-penpot-frames` 校验 23UC+5P 缺失即 exit1 / `build-overlay-report`）+ 11 单测全绿、biome + tests-tsconfig clean；②固化 [`penpot-tokens.json`](docs/design/tokens/penpot-tokens.json)（83 token）+ 23 UC frame + 5 P frame + [`overlay-report.html`](docs/design/reports/overlay-report.html)（28 节双栏、`23/23 组件、5/5 页面`、0「未生成」）；③[`design-overlay.spec.ts`](e2e/visual/design-overlay.spec.ts)（独立 `playwright.design-overlay.config.ts`，webServer 起编辑器 SPA）真机 **28/28 通过**产出全部前端截图。**补画缺口**（用户决策"补画而非 rescope"）：设计轨道原只 15/23 UC 独立板，补画 UC-003/006/007/008/010/011/012/023 八板（绘于 Penpot 'S6 — 组件视觉稿'页，忠实还原 ui-spec 各态/变体，用户视觉 **sign-off 通过** EVENT-LOG `user_decision`）+ P-002 侧栏内嵌文档列表板，至此 23 UC/5 P frame 齐备。**命名统一**（用户追加需求）：Penpot 残留 C-NNN→UC-NNN，重命名 9 板 + 16 文本标签，全库复扫 0 残留。**token-diff 目标纠偏**（前置内联决策）：比对源 `packages/themes/*/tokens.ts`→`apps/editor/src/styles/tokens.css`（editor-UI 设计系统 83 token 1:1，非内容主题）。**跨界经验**：Penpot 插件仅**当前打开页**图形可 mutate（`name`/`characters` setter + `export` 均是），跨页静默失效（同调用读回旧值）→逐页 `openPage`（最终一致，下次调用生效）后于当前页 rename+export，base64 累积落宿主 `tool-results/*.txt`→Bash 解码入库（记忆 [[penpot-plugin-gotchas]] §2）。前批 **T-104 深色主题 token 草稿已合（[PR #64](https://github.com/lync-cyber/wechat-flow/pull/64)）** —— 设计门禁链恢复后首卡（暖炭底 `#1C1917` + 提亮墨绿 `#5AA491`，34 深色 token WCAG AA 客观校验）。更早批 T-128 五主题内容元素样式（[PR #63](https://github.com/lync-cyber/wechat-flow/pull/63)）、T-064 wire-in（[PR #62](https://github.com/lync-cyber/wechat-flow/pull/62)）；各卡细节见 EVENT-LOG 与 PR 记录。
- 下一步行动: **设计门禁链已恢复并推进中**（Penpot infra 回归：`localhost:9001`=200、单实例修复后 MCP round-trip 正常）。链条 `T-104✓→T-130✓→T-131→T-113`。**下一卡 = T-131 [VALIDATION] 前端 vs Penpot 一致性门禁**（deps[T-130✓,T-104✓]）—— 运行 `pnpm design:token-diff`（AC-001 硬校验：已实测 83 token 0 mismatch exit0；不得 `[ASSUMPTION]`/`[ENV-LIMITATION]` 豁免）+ 逐 UC/P 节视觉 sign-off（[`overlay-report.html`](docs/design/reports/overlay-report.html) 28 节双栏已就绪，`23/23`+`5/5`、0 未生成）；token 不一致即阻断 Sprint 6 DONE，视觉残差登记 `conditional_release.blocking_conditions` 逐条闭环→ **T-113**（Sprint 6 验证门，14 项）→ sprint-review 收尾。节奏：单卡一 PR、串行合并（§项目状态 单写区）。设计轨道 T-095..T-106 全 sign-off（含 T-104）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟（配套 Linux 视觉基线重播）；② T-085 skill bundle description 自动 optimizer 未跑（无头 `claude -p` 401）；③ **S6 deliverable/落点漂移聚合**（sprint-review 汇总 drift-rate，勿逐卡回改；root_cause=upstream-caused）；根因已打包上游反馈 `docs/feedback/fb-dev-plan-path-drift-writeback-20260701.md`。**T-113 forward-drift**：AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
