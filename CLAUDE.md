...
- 当前阶段: development
- 上次完成: **Sprint 6 T-068 夜间模式风险预览（feature 分支 `feature/t068-night-mode-warning`，本 PR 待合 main）** —— 重锚 PRD F-002 AC-003/004：PreviewPane toolbar night-mode toggle（`off`↔`risk-preview`）+ 预览承载面深色底（`preview-surface--risk`，仅承载面变深、不改作者 HTML）+ `NightModeWarningBanner`（dismiss + sessionStorage，不写 IndexedDB）。**卡片漂移裁决（orchestrator，PRD 权威）**：原卡 `currentTheme.meta.dark → banner` 与代码库既有 `nightMode` prop + PRD 冲突（`themeMetaSchema` 无 `dark` 字段、且混淆"深色主题"与"客户端夜间反转滤镜"），按 PRD→dev-plan 权威序重锚到既有 `nightMode` 模型，已回写 dev-plan T-068 AC。根 vitest 36 绿（Banner 8 + PreviewPane 28）+ `components/editor` 70 绿，vue-tsc 0、biome 0。`wiring_complete=false` 记 sprint-review MEDIUM（banner 自包含、toolbar toggle 可达，待核运行态可见性）。**实测教训**：editor 组件测试只有经根 `vitest.config.ts`（`environmentMatchGlobs: apps/editor/** → happy-dom`）才有 DOM 环境；`pnpm --filter editor` cwd 命中无 test 块的 `apps/editor/vite.config.ts` 退化 node 环境致 `document undefined`，须用根 vitest 跑。前批 T-090 已合 main（[PR #37](https://github.com/lync-cyber/wechat-flow/pull/37)）。
- 下一步行动: **Sprint 6 远未收口（前序状态"就绪队列见底"经代码交叉核验失实，已纠正——11 个交付物抽查全 MISSING）。用户已确认：① 继续推进就绪任务；② Playwright 本机可跑无头 → T-058 谱系（T-062/063/070/113）全部解禁**。执行顺序：剩余 P0 = T-086（性能 benchmark，独立）+ T-058（视觉回归 keystone，解锁 T-062/063/070）→ P1（T-064/066/071/087/088/089/085）→ P2（T-067/069/072）→ T-113 验证门。Playwright 仅需 `npx playwright install chromium`（`playwright@^1.61.0` 已在依赖树）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
