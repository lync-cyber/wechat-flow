...
- 当前阶段: development
- 上次完成: **Sprint 4 输出能力切片 T-030+T-031 done（feature/sprint-4-output-capability，未提交）**. 落点经用户确认在 `apps/editor/src/use-cases/`（非 dev-plan 的 packages/core/src/composers/，已漂移）. **T-030 composeCopy**: pipeline composeRender→simulatePaste→buildDualMimePayload→clipboard.write + notify 回调(Toast UC-011 占位) + AC-004 接线 EditorShell.onCopyHtml→TopBar `:on-copy` + 3 行为级 wiring 测试; 收口 deferred(3) postPaste(composeCopy 已 invoke simulatePaste, AC-005 断言). **T-031 composeExportHtml**: standalone HTML(DOCTYPE/head reset+字体 CDN/body inline). 门禁地面真值: vitest 666/58 / biome 清 / tests-tsc 0 / editor vue-tsc 0(无缓存) / turbo 47/47. REFACTOR skip. **过程教训**: 并行子代理 + 各自 git stash 验证导致 harness 诊断捕获到中途陈旧快照(Cannot find module / 类型错), 主线程取地面真值复核后判定全为过期, 不可轻信. 前序: **L3 cascade T-118→T-121 已合 main**(PR #6); **framework 0.12.0**(PR #7). **运行学习(精炼)**: ①接线类 AC 须行为级测试(本切片 AC-004 初版漏测, continuation 补) ②standard 跨包卡子代理易 truncation ③并行子代理勿各自跑 git stash(竞态污染工作树/诊断) ④白名单 SSOT=packages/core/src/registry/css-property-whitelist.ts. **遗留技术债(defer)**: (a) juice/client 跨运行时 bundle(bundler-alias); (b) AC-T121-003 偏弱+transformToHast.options 占位; T-122(register_variant)随 T-036(MCP stdio)defer.
- 下一步行动: **输出能力切片 dev 完成, 待用户决定 commit + 开 PR 合 main**（分支 feature/sprint-4-output-capability, 当前未提交; 工作树含 copy.ts/dual-mime-payload.ts/export-html.ts/EditorShellCopyWiring.test.ts/compose-copy.test.ts/compose-export-html.test.ts/EditorShell.vue 改 + CLAUDE.md/EVENT-LOG）. PR 后 sprint-review 补 dev-plan amendment(T-030/031 落点 composers/→apps/editor/use-cases/). **后续切片**(用户选): MCP server(T-036→T-037/038→T-122)、relay 后端(T-032..035/091)、主题 template T-092、DESIGN(T-102/105/106)、编辑器 UI(T-040/041/042/093)、VALIDATION T-111.
...
- 当前Sprint: **Sprint 4 进行中** — L3 cascade 切片(T-118→T-121)已合 main(PR #6); 输出能力切片(T-030+T-031)**dev done 待提交/PR**. (Sprint 0/1/2/3 已关闭已合 main; PR #1/#2/#6/#7 均已合)
- 待办(deferred, 跨 Sprint): (3) ✓已清=T-030 composeCopy invoke simulatePaste 收口 postPaste true端路径(AC-005 断言); (4) MEDIUM [T-010 R-001] iframe sandbox XSS阻断 happy-dom 假绿→Playwright E2E(T-058); (a)/(b) 见 §上次完成 技术债; LOW 历史卷描述过时(知会不改: s1 T-002 AC-003 juice 旧配置 / s0 T-004 AC-006 计数 23, tool-count 已锚 T-118 AC-005). 上游反馈: #253(已闭环)/#254/#255 + fb-framework-update-0_11_1 待提交 CataForge; 历史 #234/#235/#236
- 文档状态:
  - prd: approved
  - arch: approved
  - ui-spec: approved
