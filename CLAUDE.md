...
- 下一步行动: **Sprint 5 进行中 — 就绪代码批 T-080/T-116/T-117/T-049 已串行 TDD（light-dispatch）逐个闭环，[PR #26](https://github.com/lync-cyber/wechat-flow/pull/26)（squash `91a7b0d`）已合 main、本地 main 同步、`feature/sprint5-ready-batch` 已删除**。独立验证：全量 `pnpm test:coverage`=130 files/**1611 passed**/1 skip、**Branches 90.67%(1984/2188) ≥ 90% 阈值**、逐包 typecheck 全 0（core/contracts/mcp-server/cli/plugin-api/editor）、module-boundaries 0、arch-guard depcruise 0 error（8 warn 均预存 no-orphans）、biome 0、**pre-push 守卫全绿（arch-guard/coverage/gen-templates/typecheck）**（唯一 flaky `tests/relay/headless-render.test.ts` chromium hook timeout 系基线，零回归；首推因 batch 新文件 error/备选分支未覆盖致全局 branches 89.4%<90% 被 coverage 守卫拦，补 14 条边界测试至 90.67% 解除）。**T-080**(HELD 解封) list_tokens/describe_token thin-wrapper 接 token 注册表(61 token/五类)；24 tool 全实现后哨兵测试换名(dispatchTool 合成名测 E_NOT_IMPLEMENTED；server valid-user 路径改断言成功响应,10 测试) · **T-116** apps/cli init+validate(commander 入口；validate 自实现 manifest `E_MANIFEST_INVALID: missing required field 'name'` + variant 一致性 + 主题守护；命令纯函数化便于单测,4 测试)；orchestrator 补 plugin-api barrel re-export 修 module-boundaries 深引用违规 · **T-117** 渲染壳 dev/publish/render/copy/export thin wrapper(委托 renderMarkdown/simulatePaste，CLI 自建 bootstrap 注册 5 主题包；publish SHA256 比对本地 state；dev 可注入 watcher + formatHmrMessage,27 测试) · **T-049** 主题继承 delta-merge(token 值替换同步到 blocks 使 delta 改色真实反映 HTML，describeTheme 接线无 extends 早返回零回归) + brand-pack lock(paintable 白名单前拦截锁定 token + warn diagnostic,4 测试)。**纠偏沿用教训**：①串行驱动避并行注册表回归(本批全程串行)；②子代理 new-diagnostics(TS2307/6133/2349) 多为并发滞后快照/IDE 异 config 误报，逐一实跑权威 `tsc -p`/vitest 确认非真实；③T-049 防 token-seed↔token 同类循环——delta-merge 用 lookup 注入不 import registry。per-task code-review 本批全 security_sensitive=false → 延迟 Sprint 5 收尾批量 sprint-review。前序 P1 批 T-048/T-075/T-123/T-078/T-079 [PR #25](https://github.com/lync-cyber/wechat-flow/pull/25)(`f08afe2`)、并行实现批 T-047/T-051/T-073/T-074/T-077 + MCP 批 T-081~T-084 [PR #24](https://github.com/lync-cyber/wechat-flow/pull/24)(`55a8de1`) 已合 main。**T-103 DESIGN ✅（2026-06-29 用户最终视觉 sign-off 通过 → 解封 T-055）**：P-005 移动端只读预览高保真视觉稿绘于 Penpot 'S5 — 组件视觉稿'页，含 375px(populated 渲染态文章·serif 标题 Noto Serif SC + 诗体 + 品牌赏析 callout + 配图占位) 与 320px(拇指热区验证 PS-009·双层弧舒适0.22/可达0.10 + 虚线边界 + 支点→复制按钮中心可达连线) 双 Frame；AC-001/002/003 程序化校验全过(底栏 56px·文档切换器 select·复制按钮 80×44≥44×44·find_shape 命中 2 board)；经 /frontend-design 审查在锁定设计系统内(Noto 字族/品牌绿/ui-spec token)应用 6 项改进(serif 编辑字体/去 emoji/depth 阴影/顶栏文档名主角/select 语义 chevron/拇指弧强化)；EVENT-LOG user_decision approved 记录(ref dev-plan-wechat-flow-s5#§T-103)。**T-055 ✅（2026-06-29 light-dispatch TDD 闭环，分支 `feature/sprint5-t055-mobile-preview`，经 [PR #27](https://github.com/lync-cyber/wechat-flow/pull/27) squash `91657d9` 已合 main、`feature/sprint5-t055-mobile-preview` 已删、本地 main 同步）**：P-005 移动端只读预览代码实现——`PreviewPage.vue`(完整重写占位:简化顶栏48px + PreviewPane 375px 视口 iframe + MobileBottomBar + DocumentListSheet，从路由 :docId 经 editor store loadDraft)、`MobileBottomBar.vue`(fixed 底栏56px·`--z-mobile-bar`·文档切换器左+primary复制按钮右触控区≥44)、`DocumentListSheet.vue`(P-002 底部抽屉·listDocuments 数据源·`--duration-base` 滑入·max 60vh)、`mobile-copy.ts`(use-case:clipboard 可用走 composeCopy+「已复制」/不可用降级 textarea+execCommand+「请手动长按复制」warning)、`use-toast.ts`(Toast.type 加 warning)。AC-001~004 全覆盖。**orchestrator 审计捕获并消重**:implementer 自报 refactor_needed=false 漏判 mobile-copy.ts 孤儿(handleCopy 内联重写其全部逻辑致 duplication+dead-code)→ handleCopy 委托 mobileCopy()(从孤儿转生产接线)+补 execCommand 守卫+删测试孤儿 vi.hoisted(TS6198)。独立验证:新测试21 PASSED、editor 全量45 files/403 PASSED 零回归、vue-tsc 0、biome 0。LOW(延 sprint-review):移动端成功复制路径双 toast(composeCopy 内部「已复制到剪贴板」+mobileCopy「已复制」),根治需 composeCopy 返成功布尔超 P2 范围。**Sprint 5 进入收尾阶段（全部代码任务 ✅ 合 main，含 T-055/T-103 经 PR #27）：剩 ① 批量 sprint-review（per-task code-review 延迟至此聚合，覆盖本 Sprint 全部已合任务 AC + 范围偏移检测）② T-112(VALIDATION，user_facing_critical_path,依赖项全 ✅，需用户手动验证 CLI/zh-typo UI/MCP transport/模板市场/素材上传/24Tool) ③ wiring backlog(deferred,不阻塞收尾)**。Sprint 5 全任务卡见 [`dev-plan-wechat-flow-s5.md`](docs/dev-plan/dev-plan-wechat-flow-s5.md)。人工检查点=[pre_dev]（已过），post_sprint 非门禁。framework-feedback 已上报 [#374](https://github.com/lync-cyber/CataForge/issues/374)·[#375](https://github.com/lync-cyber/CataForge/issues/375)·[#376](https://github.com/lync-cyber/CataForge/issues/376)。
...
- 待办(deferred)（✅=deferred backlog 已解决并合 main, PR #18）:
  ...
  - **Sprint 5 wiring backlog（并行实现批 T-047/T-051/T-077 deferred 生产挂载，unit AC 已过、模块未挂进程入口）**: T-051 admin 路由调和现有 createApp 的 JWT-scope admin 模型(现 501 占位) + HTTP transport 进程启动入口(@hono/node-server)·T-077 wechat-assets route 挂 createApp(经 auth 注入真实 apiKeyId，现 enqueue 空串) + job-worker 注册 `wechat-asset-upload` worker + 微信 access_token 获取流程(/cgi-bin/token 带缓存) + 真实 imageUrl→multipart 上传 + uploader 结果 url 改用响应 body.url(修 access_token 泄漏，CODE-REVIEW-T-077-r1 R-001) + imageUrl SSRF/https 校验(R-004)·T-047 真实 Worker bootstrap(启动期删除网络全局 + Comlink defineApi，现 runtime.ts 为骨架)·**T-117 dev 命令真实 Vite dev 进程 bootstrap**(现 `serverFactory` 为可注入占位骨架，watch+HMR 日志契约已单测；真实 Vite middleware + pack live-reload 待接 @wechat-flow/editor 的 Vite 6 或独立 createServer)。
