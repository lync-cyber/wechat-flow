...
- 上次完成: **BC-11 设计一致性收敛（UC-021 二段式补全流程，backlog HIGH，PR #95）**：① 补二段式选择流——block 项点击进 variant 面板（面包屑「← blockId」回退列表一段）+ 参数表单（读 `attrsSchema.shape` 前 3 字段，超 3 项跳「在 InsertDrawer 中配置」onOpenInsertDrawer）+「插入」后段，inline mark 直选无二段；② 分类 tab 行（行内/块级，数据驱动依 registry 内容非硬编码）、block 项 variant 计数角标、行首图标（新 `block-glyphs.ts` 40 block glyph + inline/block fallback）、typing 匹配段高亮 span；③ snippet 契约改写——variant 经 directive class 语法 `:::block{.variant}`（default 与省略等价），inline 参数移出方括号 `:mark[]{k=v}`、含空格值加引号，SourcePane onSelect 载荷升为完整 SnippetOptions、cursorOffset 动态 indexOf 定位新格式落点。浏览器实链验证（`:::` 触发 40 候选+tab+图标+角标 / 点 heading→variant 面板+面包屑+参数 / 选 underline 插入 `:::heading{.underline}` / `:bad`→inline 直插 `:badge[]` 全通、零 console 报错）+ 全仓门禁绿（vitest 2800/1 skip、editor vue-tsc、tests tsconfig、biome）。**Phase 1-7 + post 全部完成。**
- 下一步行动: **项目 completed，剩余均为用户侧决策**（upstream 三件套已提报 #421/#422/#423，见 §待办）：① release go/no-go——包版本核对（现全 0.0.0）、mcp-server 是否纳入发布面（现 private:true，翻转后 `npx @wechat-flow/mcp-server` 才真正可用，发布集 13→14 需同步 deploy-spec §9-D1）、GitHub Environment npm-publish required-reviewers 配置、PRD §3.5 真实公众号粘贴回归、Docker 环境容器 bring-up 实测、CVE 门禁阈值（deploy-spec §9-D4 needs_input）② **CataForge 0.15.0 升级可用**（本地 editable 包已 0.15.0、scaffold 仍 0.14.0，doctor 报 context.mode hybrid 枚举失效 + 5 处引用逃逸 FAIL 均属升级漂移）——运行 framework-update skill 对齐四层后再做后续会话 ③ **设计一致性收敛 backlog**（T-131 AC-004 二轮 sign-off 产出 15 条 BC，两 HIGH（BC-1/BC-11）已收敛，余 13 条 open，见 §待办 + SPRINT-REVIEW-s6-r3.md）——MEDIUM 批为图标体系/组件级结构收敛（developer），BC-12/13 已裁转设计侧并入 BC-15，UC-014 设计帧重导出 + BC-15 设计侧对齐随 amendment 批走；其余 dev 残留（T-033 env-gated、R-007、T-124/T-126 需真实凭据、strip 双规则独立会话）见 §待办 ④ arch 措辞修订三项已登记 §待办，随 framework-update 后的 amendment 批走。
