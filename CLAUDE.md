...
- 当前阶段: development
- 上次完成: **T-129 桌面浏览器 Clipboard execCommand 降级路径（[PR #61](https://github.com/lync-cyber/wechat-flow/pull/61)）** —— PRD→任务 AC 级追溯审计标出的唯一真实 gap 闭环（用户裁决：设计门禁链受 Penpot MCP 不可达阻塞，转做非设计门禁 backlog）。`apps/editor/src/use-cases/copy.ts` 的 `composeCopy` catch 此前仅弹「复制失败」error Toast、无降级；现按 `window.innerWidth >= 768` 分流：桌面走新增私有 `fallbackCopyToClipboard`（隐藏 textarea + `execCommand('copy')` 拷 plainText，position:fixed 移出视口 + try/finally 保 removeChild，复用 `mobile-copy.ts` 范式）+ warning Toast（文案含 Ctrl/Cmd+C，「如未生效」hedge 不虚假宣称成功）；非桌面维持 error Toast 交由 `mobile-copy.ts`（AC-004 边界隔离，两条降级不重叠）。`ComposeCopyInput.notify` 类型联合放宽含 `warning`（调用点 `EditorShell.vue:211` pushToast 已支持）。**执行**：light-dispatch（standard 模式 light 任务，非 inline）；4 AC 行为断言全绿（execCommand 调用+textarea 清理+不抛异常 / warning 文案 / 成功路径不触发降级 / 移动端不走桌面分支）+ mobile-copy 回归 2/2 + 全仓 2275 tests。**主线程独立复核**（不轻信子代理自报）：vitest 6/6、editor vue-tsc 直跑绕 turbo exit 0、biome exit 0、根 tests/tsconfig exit 0，读实现确认非空心断言。`execCommand` deprecated（TS 6385/6387）是任务卡认可的预期兜底 LSP 提示、非编译错误。前批 T-130/T-131 视觉门禁 dev-plan amendment 已合（[PR #60](https://github.com/lync-cyber/wechat-flow/pull/60)）。
- 下一步行动: **用户裁决转做非设计门禁 backlog**（设计门禁链 T-104→T-130→T-131→T-113 待 Penpot 环境就绪恢复）。剩余可离线推进的 backlog（不依赖 Penpot）：**T-128**（五主题内容元素选择器样式增强 list/ul/ol/li/img/table/a，deps[T-059]，纯 CSS 守卫增强）· **T-064** `use-auto-backup` 待 wire 进 EditorShell（参 T-069 preferences-store 范式，wiring-completeness 遗留）。节奏：单任务一 PR、串行合并（§项目状态 单写区）。**设计门禁链恢复前置**：Penpot docker 栈起（`localhost:9001/mcp/stream` 现 HTTP 000 不可达）+ 会话重启加载 penpot MCP（.mcp.json 需重启生效）；恢复后执行顺序 = **T-104**（Penpot 深色主题 sign-off，需用户视觉）→ **T-130**（一致性工具链，依赖全部 T-095..T-106 + T-104 导全量 frame/token）→ **T-131**（视觉门禁 + 用户 sign-off，T-113 前置）→ **T-113**（Sprint 6 验证门）→ sprint-review 收尾。设计轨道 T-095..T-106 除 T-104 外 11 张稿均已 sign-off（EVENT-LOG 实证），`docs/design/` 产物固化归 T-130。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 use-auto-backup 待 wire；② T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟（配套 Linux 视觉基线重播）；③ T-085 skill bundle description 自动 optimizer 未跑（无头 `claude -p` 401）；④ **S6 deliverable/落点漂移聚合**（sprint-review 汇总 drift-rate，勿逐卡回改；root_cause=upstream-caused）；根因已打包上游反馈 `docs/feedback/fb-dev-plan-path-drift-writeback-20260701.md`。**T-113 forward-drift**：AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
