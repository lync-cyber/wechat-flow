...
- 上次完成: **dev 残留收口批③（本 PR）**：/metrics Prometheus 端点 — arch#§5.5 三项 SLI 全落地（deploy-spec §2.3 登记项清零）：mcp-server `GET /metrics`（`render_markdown_latency_ms` histogram 包 renderMarkdownTool + `paste_simulation_diff_ratio` histogram 包 simulatePasteTool，ratio=nodeDiffs/sourceNodeCount 下界、core `SimulatePasteResult` additive 增 `sourceNodeCount`）+ relay `GET /metrics`（`job_queue_depth` gauge 带 kind label，`createJobsRuntime().queueDepths()` 按 `ALL_JOB_KINDS` 取 BullMQ getWaitingCount，main.ts 已接线）；prom-client、独立 Registry、端点不挂 Bearer。前批 批①②（[PR #81](https://github.com/lync-cyber/wechat-flow/pull/81)/[#82](https://github.com/lync-cyber/wechat-flow/pull/82) 已合并）。**Phase 1-7 + post 全部完成。**
- 下一步行动: **项目 completed，剩余均为用户侧决策**（upstream 三件套已提报 #421/#422/#423，见 §待办）：① release go/no-go——包版本核对（现全 0.0.0）、mcp-server 是否纳入发布面（现 private:true，翻转后 `npx @wechat-flow/mcp-server` 才真正可用，发布集 13→14 需同步 deploy-spec §9-D1）、GitHub Environment npm-publish required-reviewers 配置、PRD §3.5 真实公众号粘贴回归、Docker 环境容器 bring-up 实测、CVE 门禁阈值（deploy-spec §9-D4 needs_input）② **CataForge 0.15.0 升级可用**（本地 editable 包已 0.15.0、scaffold 仍 0.14.0，doctor 报 context.mode hybrid 枚举失效 + 5 处引用逃逸 FAIL 均属升级漂移）——运行 framework-update skill 对齐四层后再做后续会话 ③ dev 残留见 §待办（T-131[C] + S6 arch 收口项 + Sprint 5 backlog；Phase 7 登记的 bin 字段/engines/JobsClient//metrics 四项全部收口）。
