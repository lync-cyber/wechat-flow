...
- 当前阶段: development
- 上次完成: **Sprint 6 T-062 CI 任务图完整配置（[PR #59](https://github.com/lync-cyber/wechat-flow/pull/59)）** —— chore/config，权威 **arch#§7.4**。**产出**：① `ci.yml` 补 workflow 级 `env: TURBO_TOKEN/TURBO_TEAM`（arch#§7.4 远程缓存；缺 secrets 时 turbo 优雅回落本地 `.turbo`）+ 清理 quality-gate 旧变更叙事 comment 改注为 arch 任务图映射；② 确认 arch#§7.4 全部门禁在跑 —— quality-gate（`pnpm lint` biome + `pnpm typecheck` 全量含 `tests/tsconfig` + `pnpm test:coverage` 根 vitest 覆盖 ruleset-fixtures/主题 9 维/覆盖率阈值）· `cross-runtime` job（T-063 预置）· `guards`（arch/boundaries/secrets）· `visual-core`+`visual-sampled`（PR-only + playwright + `if:always()` 上传 `playwright-report/` 含 diff）；③ `turbo.json` 任务图已含完整 7 阶段（含 `visual-regression` pipeline），无需再改。**关键裁决**：卡片 AC-002..007 的 `pnpm -r <cmd>` 具体命令与实际 scripts 不兼容且**会 regress**（漏 `tests/tsconfig` / 根 `tests/**` / coverage 阈值；`--filter ruleset`/`theme-core` 包名不存在，测试集中在根 `tests/`）→ 以 arch#§7.4 概念任务图为权威、不为对齐字面 AC 降级；job 保持合并结构（用户裁决，免 6× install + 质量回归）。**兜底**：6 个 workflow YAML python 全 OK · `turbo --dry` 任务图解析正常（env 不破坏）· 本任务无代码变更（仅 CI env + 注释 + 文档），门禁沿用 PR #58 全绿基线。deliverable 路径漂移（`visual-regression.yml`→`visual-core/sampled`、视觉测试实路径 `e2e/visual/`）登记 §漂移聚合 ④。前批审计 [PR #58](https://github.com/lync-cyber/wechat-flow/pull/58) 已合。**产出**：① `tests/cross-runtime/fixtures.ts` —— 3 fixture（CJK / block-directive / frontmatter）+ `computeFixtureHashes(render?)`（side-effect 注册 blocks/marks + `registerTheme(defaultTheme)`，crypto.subtle SHA-256）+ golden `EXPECTED_HASHES`（node 实测生成）；② `node/browser/worker/edge.test.ts` 四 target 均 `expect(hashes).toEqual(EXPECTED_HASHES)`（全等 ⟹ 四运行时字节一致）+ `worker-entry.ts`（`new Worker(new URL(...),{type:module})` 内渲染回传）；③ `vitest.cross-runtime.{node,browser,edge}.config.ts` 三专用配置（edge = `defineWorkersConfig` Miniflare `compatibilityDate 2024-09-01`+`nodejs_compat`；browser = `@vitest/browser` playwright chromium）；④ `scripts/gen-cross-runtime-hashes.ts` + `test:cross-runtime`/`gen:cross-runtime-hashes` scripts；⑤ 根 `vitest.config.ts` 排除 `tests/cross-runtime/**`（专用配置驱动，避免默认 node runner 误跑）；⑥ CI `cross-runtime` job（Playwright chromium 安装 + `test:cross-runtime`）。**关键裁决**：(a) deliverable 路径 `packages/core/src/__tests__/` → `tests/cross-runtime/`（根 tests/ 由 tests/tsconfig 独立管辖，置 src 下会被 coverage/typecheck/biome 当源码）；(b) **worker 兼容修复** —— `decode-named-character-reference`（micromark 依赖）`browser` 入口模块顶层 `document.createElement`，Web Worker 无 document 即抛；其 node 入口静态映射解码、输出字节一致，browser 配置 `resolve.alias` 强制指向 node 入口（`require.resolve` 版本无关）；(c) 单实例不变量 —— fixtures 相对引 core + blocks 内部包名引 core 经 Vite real-path 去重同实例，避免主题注册表分裂。**兜底（主线程实证）**：`test:cross-runtime` 6 测试 / 4 运行时全绿（edge workerd 18.6s 通过）；完整套件 191 files / 2271 pass + 1 skip 零回归；typecheck 50/50 + tests/tsconfig 0；biome 700/0；guard:arch 0 error（2 条既有 no-orphans WARN 未触碰）+ guard:boundaries 478/0。依赖 `@cloudflare/vitest-pool-workers@0.5.41` + `@vitest/browser@2.1.9`（复用 T-058 playwright 1.61）。曾遇后台 `pnpm add` 竞态覆写 package.json scripts（已补回）。per-task code-review 延迟至 sprint-review，主线程已 inline 审全部 diff。前批 T-070 已合（[PR #56](https://github.com/lync-cyber/wechat-flow/pull/56)）。
- 下一步行动: **Sprint 6 就绪 P0 全部完成（T-062 合后）**。余未实现 = **T-104**（Penpot 设计，需用户视觉 sign-off）+ **T-113**（验证门，见下 forward-drift）+ deferred backlog（T-128/T-129 等）。下一步 = **T-113 验证门** 或 **T-104**，之后 Sprint 6 收尾 sprint-review（聚合 §漂移 ④ + wiring-completeness 遗留）。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 `use-auto-backup` 待 wire 进 EditorShell（**注**：T-069 已把 `preferences-store` wire 进 EditorShell，可参照其 props/init 范式补 T-064）；② **T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟**（注入机理/PaintDrawer/测试均就绪，5 内置主题暂留 `{}` 守视觉基线，激活需配套 Linux 视觉基线重播评估）；③ **T-085 skill bundle description 自动 optimizer 未跑**（环境无头 `claude -p` 认证 401 限制，已主线程亲评优化 2 触发盲点；带 API key 或 CI 环境可补跑 run_loop 做 3-run 统计验证；skill-creator `run_loop.py` 的 Windows GBK `read_text()` 编码 bug 值得上游反馈）；④ **S6 deliverable/落点漂移聚合（sprint-review 汇总成单条 drift-rate，勿逐卡回改路径；root_cause=upstream-caused 计划路径未回写，代码均正确、功能完整）**：T-063 `src/__tests__/cross-runtime/`→`tests/cross-runtime/`·T-070 `src/__tests__/{version,deterministic}.test.ts`→`tests/core/`·T-059 `guard/eight-dimensions.ts`→`guard/nine-dimensions.ts`（8→9 维）·T-064 `editor/stores/*`+context_load M-007/API-022→core M-013 持久化层·T-066 StatusBar→`components/layout/`+context_load M-007/API-006→M-001·卡片惯引 `packages/mcp-server/`→实际 `apps/mcp-server/`、`components/X.vue`→`components/editor/X.vue`。处理对齐 S4 SPRINT-REVIEW-s4-r1（deliverables 代表性非穷举，仅记 drift-rate 不回改）；根因已打包上游反馈 `docs/feedback/fb-dev-plan-path-drift-writeback-20260701.md`。**T-067 seam 已接线闭环**（T-069 已 wire `inputRulesExtension` 进 use-codemirror via SourcePane props + store 谓词）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
