...
- 当前阶段: development
- 上次完成: **Sprint 6 T-064 多文档管理 + 自动备份精化（[PR #43](https://github.com/lync-cyber/wechat-flow/pull/43)）** —— 文档持久化属 M-013，实现落 `@wechat-flow/core` 持久化层（**非** editor store，卡片 `document-store.ts`/`M-007`/`API-022` 漂移已裁决）：`indexeddb-adapter.ts` DB_VERSION 1→2 + 新 `backups` store（index `by_docId`，upgrade 按 oldVersion 增量迁移）+ `BackupRecord` + `DocumentMeta.size`（TextEncoder 字节数）；`backup/auto-backup.ts` 的 `createBackup`/`listBackups`/`deleteBackupsForDoc` + 保留策略（`MAX_BACKUPS_PER_DOC=5` 删最旧）；`manager.ts` `deleteDocument` 级联删备份 + `duplicateDocument`；editor `use-auto-backup.ts`（5min 间隔 + dirty 触发 + `onUnmounted` clearInterval）。**主线程兜底**：① implementer self-report `refactor_needed=false` 漏判 `manager`↔`auto-backup` 循环 import（coupling），主线程解耦（auto-backup 改经 adapter 直读 documents store）；② IDE 诊断陈旧缓存假报未导出/缺 size，亲跑权威 tsc 证伪。`use-auto-backup` 未 wire 进 EditorShell（`wiring_complete=false`，非 critical_path，登记 sprint-review §wiring-completeness 记 MEDIUM）。vitest 167 文件/2062 绿（+18：backup 12 + use-auto-backup 6）、core/editor/tests typecheck 0、全仓 biome 0。前批 T-058 已合 main（[PR #41](https://github.com/lync-cyber/wechat-flow/pull/41)，linux 基线已播种）。
- 下一步行动: **Sprint 6 续建中。下一批 P1 剩余 = T-066/071/087/088/089/085（T-064 已完成 [PR #43](https://github.com/lync-cyber/wechat-flow/pull/43)）→ P2（T-067/069/072）→ T-113 验证门**。下一任务 = **T-066（撤销/重做 + 查找替换 + 字数统计，deps[T-009,T-012] 就绪）**。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**T-064 遗留**：`use-auto-backup` 待 wire 进 EditorShell（登记 sprint-review §wiring-completeness）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
