...
- 当前阶段: development
- 上次完成: **Sprint 6 T-088 编辑器 paint drawer + color picker 双向绑定 frontmatter（[PR #49](https://github.com/lync-cyber/wechat-flow/pull/49)）** —— 属 **M-001**（F-003 AC-010）。**调查裁决**：`theme-override.ts` `getPaintableSet` 仅 array 形式生效（object→空 set），5 内置主题 `paintable:{}`，`applyPaintToBlocks` 仅当 frontmatter 有 paint 时查 → **不填充内置主题 = 零渲染/视觉基线影响**（保守，AC 由测试内带 `paintable:string[]` 主题验证）。**core 新增**（additive，不改 `parseFrontmatter`）`upsertFrontmatterPaint`（`yaml` 同包 `stringify`，解构 drop paint 避 noDelete + 重组源码）。**editor**：`use-paint-binding`（`paintableTokens` array 分支 + `currentPaint` parseFrontmatter computed + `setPaint`/`clearPaint`），**双向无环**（写经 store.content→currentPaint 重算→picker `:value` 单向更新，store `content===markdown return` 防重入）；`PaintDrawer.vue` 原生 `<input type=color>`；ContextMenu「自定义配色」→ EditorShell `isPaintDrawerOpen` **端到端接线**（非延迟 wiring）。AC-004 经既有 `applyPaintToBlocks` `paint-token-not-paintable` warning + DiagnosticsPanel 黄色。**兜底**：全量 vitest 177 文件/2147 绿（+23）含 AC 端到端（picker↔frontmatter 双向 + 真 renderMarkdown 验 warning）；pnpm typecheck 50/50（含 vue-tsc）、tests-tsc 0、biome 667/0；CI 5 检查全绿。多条 new-diagnostic 假报（stringifyYaml 未用 / pinia 找不到 / PaintDrawer.test import 路径）经 vue-tsc+tests-tsc 0 + 眼检 frontmatter.ts:43 + `ls` 证无 orphan 全证伪（陈旧快照）。前批 T-087 已合（[PR #47](https://github.com/lync-cyber/wechat-flow/pull/47)）。
- 下一步行动: **Sprint 6 续建中。下一批 P1 剩余 = T-089/085（T-064/066/071/087/088 已完成）→ P2（T-067/069/072）→ T-113 验证门**。下一任务 = **T-089（packages/ruleset 关键词 lint + 可热更新词库，M-003，F-011 AC-007）**。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 `use-auto-backup` 待 wire 进 EditorShell；② **T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟**（注入机理/PaintDrawer/测试均就绪，5 内置主题暂留 `{}` 守视觉基线，激活需配套 Linux 视觉基线重播评估）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
