...
- 上次完成: **F-003 夜间风险 render pipeline 打通 + 职责归位（本 PR）**：新增 `packages/core/src/pipeline/readability.ts`（`collectNightRiskIssues`：inlineStyle 后遍历节点算 WCAG 对比度 <4.5 产 NightRiskEntry，nodeSelector 去重、非 hex 跳过），render.ts 接入并成为 DiagnosticReport 唯一组装者；applyRuleset 职责归位——`ApplyRulesetResult` 收窄为 `{hast,diagnostics,nodeChangeRecords}`，删除它算不出的伪 nightRiskIssues 与硬编码 versionTriple（真值组装上移 render.ts）；F-006 清理 `PAINTABLE_SEMANTIC_TOKENS` 的 `--color-success` 死项。全量绿 typecheck 50/50 + vitest 2846 passed + biome 794 clean（唯一失败=chromium 冷启动 flake #35 与改动无关）。TDD standard（RED→GREEN，REFACTOR 无触发）。
- 下一步行动: **项目 completed，剩余均为用户侧决策**：① **release go/no-go**（包版本 0.0.0 核对 / mcp-server 发布面 private 翻转→发布集 13→14 / npm-publish reviewers / 真实公众号粘贴回归 / Docker bring-up / CVE 阈值——细节见 §待办 + deploy-spec §9）② **设计一致性 backlog**：BC-1..15 全收敛；F-003 夜间风险 render pipeline 本 PR 打通（applyRuleset 职责归位 + core/pipeline/readability.ts）；余 BC-4 可读性评级算法待 product spec、UC-015 分类 tab defer（数据契约缺 category）见 §待办 ③ **amendment 批已完成**（arch 措辞 + ui-spec 同步 + Penpot 对齐，见 上次完成）。
...
- 待办(deferred)（仅列 open 项；已解决项见 git/PR 历史）:
  - **设计一致性收敛 backlog（T-131 AC-004 二轮 sign-off；逐条见 `SPRINT-REVIEW-s6-r3.md`）**: 15 条 blocking_conditions 全 disposition。**BC-1..15 全收敛**：BC-1..11+BC-14 = PR #94–98；BC-12/13/15 = PR #101 Penpot 追平前端+用户 sign-off（见 上次完成）。**余 open**：① BC-4 可读性评级算法待 product spec（owner=product）——夜间风险 nightRiskIssues render pipeline 已本 PR 实现（`core/pipeline/readability.ts` 渲染后 WCAG<4.5 产 entry，UI 段已消费）② UC-015 分类 tab **defer（非缺陷）**：`BlockDefinition` 无 category 字段→分类无法数据驱动，硬编码违 A-014 裁决；真正前置=ui-designer 冻结分类 + 数据契约加 category（owner=ui-designer）。r3 判一致 4 节 + LOW 散点见 r3。
