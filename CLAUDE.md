...
- 上次完成: **dev 残留收口批④~⑧（[PR #84](https://github.com/lync-cyber/wechat-flow/pull/84)~[#87](https://github.com/lync-cyber/wechat-flow/pull/87) 已合并 + 本 PR）**：S6/T-091 LOW 批（SR-007/008/009·R2-001·R-006/009/010 + editor-session refreshUntil=expiresAt 语义钉死，#84）；S6① 补丁包声明式 DSL（matcher 判别联合 + 预注册 transform-id 白名单，fail-closed 校验 + 原子 compile-then-upsert，决策记录 `docs/research/tech-eval-patch-dsl.md`，#85）；S6② 42 规则 fixture 迁移 `packages/ruleset/src/rules/builtin/{rule-id}/`（25 迁 + 17 补 + 漂移守卫，附带发现 strip 双规则假绿缺陷见 §待办，#86）；Sprint 5 backlog editor 六项（export-long-image SSE 全链路 = use-sse-job 生产消费者落地、preferences init guard、readMinutes 实算、onDownloadHtml 错误反馈、JWT 主动续期、SettingsPage [ASSUMPTION]，#87）；SR-R2-003 上传进度（本 PR）：`POST /api/v1/images/upload` → `202 {jobId}` + 进程内异步处理（生产入口无 BullMQ Worker，走队列会假收口）复用既有 JobStore/SSE 桥（bullmq-store 增无队列 job 时的快照回退），editor use-image-upload 消费真实 progress，T-093 AC-002 原语义达成。前批 批①②③（PR #81/#82/#83）。**Phase 1-7 + post 全部完成。**
- 下一步行动: **项目 completed，剩余均为用户侧决策**（upstream 三件套已提报 #421/#422/#423，见 §待办）：① release go/no-go——包版本核对（现全 0.0.0）、mcp-server 是否纳入发布面（现 private:true，翻转后 `npx @wechat-flow/mcp-server` 才真正可用，发布集 13→14 需同步 deploy-spec §9-D1）、GitHub Environment npm-publish required-reviewers 配置、PRD §3.5 真实公众号粘贴回归、Docker 环境容器 bring-up 实测、CVE 门禁阈值（deploy-spec §9-D4 needs_input）② **CataForge 0.15.0 升级可用**（本地 editable 包已 0.15.0、scaffold 仍 0.14.0，doctor 报 context.mode hybrid 枚举失效 + 5 处引用逃逸 FAIL 均属升级漂移）——运行 framework-update skill 对齐四层后再做后续会话 ③ dev 残留见 §待办（T-131[C]、T-033 env-gated、R-007、真实环境 E2E、strip 双规则修复独立会话进行中；S6①②③ + Sprint 5 backlog + T-091 LOW + Phase 7 四项均已收口）④ arch 措辞修订三项已登记 §待办，随 framework-update 后的 amendment 批走。
...
- 待办(deferred)（仅列 open 项；已解决项见 git/PR 历史）:
  ...
  - **真实环境 E2E（conditional_release，不阻塞收尾）**: iframe sandbox XSS（happy-dom 假绿→Playwright，可复用 T-058 Playwright 设施）·T-124 Worker delete 全局·T-125 mcp HTTP 进程·T-126 微信真实 API（需 AppID/Secret）·T-127 vite HMR 浏览器·relay BullMQ Worker bring-up（render/export kind 经 jobs API 入队后需 Worker 消费才终结；image-upload 走进程内路径不依赖）—— 均 deferred 至部署/真实凭据环境。
