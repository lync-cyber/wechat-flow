...
- 当前阶段: development
- 上次完成: **T-111 [VALIDATION] done + 下载 HTML wiring 修复（2026-06-25，分支 feature/t093-image-upload-ui，未 commit）→ Sprint 4 全部任务完成** —— T-111 自动验证（orchestrator 搭最小可行栈：复用既有 docker Redis + relay env[JWT secret/allowed origin 5199/IMAGE_HOST=local→editor public/uploads] + 补 editor vite proxy `/api`→3000）：复制/下载 HTML、拖拽上传（浏览器 e2e：drop→占位→经 proxy session bootstrap→带 JWT 上传→sharp→local 落地→占位原子替换→图片 200 可服务）、MCP render_markdown（集成测试 11 passed；literal `dist/stdio.js` 受阻=build skip+无 key seeding）、editor session（匿名 bootstrap scope=upload，无 JWT 401）实测通过；export_long_image（需 job-worker+Playwright 管线，T-035 覆盖）+ 主题卡 pixelmatch（需 T-058 harness）环境受限推延；真实公众号粘贴 T-090 周期。验证暴露并修 **下载 HTML wiring gap**：`export-download-html` 命令原 placeholder→接 `composeExportHtml`+blob 下载（command-registry/EditorShell+测试，浏览器实测 blob 7182B/download=wechat-flow.html）。全量 vitest 1189 passed/1 skip、editor vue-tsc 0、biome 0、零 console error。前序 **T-093 UC-018 上传 UI done**（light-dispatch：`use-image-upload.ts` 注入式状态机 + 最小 `use-editor-session.ts` 匿名 bootstrap + `ImageUploadOverlay.vue` UC-018 5 态 + `SourcePane.vue` 拖拽/粘贴接线，AC-004 JWT 真实透传，35 新测试 PASS；观察留 sprint-review：progress 恒0 / dragging 浮层悬停不显 / client session composable security 维度）。前序 **Sprint 4 UI 轨 T-040/T-041/T-042 + 跨运行时主题加载修复 → [PR #20](https://github.com/lync-cyber/wechat-flow/pull/20) merged 入 main（squash fb31652）**。其中 T-042 P-004 设置页 —— light-dispatch 实现 /settings 双栏 master-detail（6 导航分组）+ ImageHostConfig（6 图床折叠卡，密码框字段）+ ApiKeyConfig（AppSecret 密码框+眼睛切换）+ `packages/core/src/storage/credentials.ts`（Web Crypto 凭据加密）。**security_sensitive code-review 把关**：r1 verdict=needs_revision 报 2 HIGH 安全（R-001 主密钥内嵌每条密文记录、R-002 主密钥 `extractable:true` 明文字节存 IDB）+ 3 MEDIUM + 4 LOW；修订**实证 fake-indexeddb 可存非可导出 CryptoKey** 后合并重设计：非可导出 CryptoKey 直存 IDB、`EncryptedRecord={iv,ciphertext}`、全程无原始密钥字节，根治 R-001/R-002，并修 clearCredential 原子化(R-003)/clear 错误处理(R-004/005)/组件实参断言(R-006)/[ASSUMPTION] 标记(R-009)。验证（orchestrator 独立、规避 turbo 缓存）：core+editor `tsc/vue-tsc` 退出码 0、目标单测 53 passed（credentials 15 含 CryptoKey extractable===false / 无 keyBytes / 密文非明文 / 跨 closeDb 持久断言）、biome 0 error、浏览器 /settings 实测 AC-001(6导航默认编辑器)/AC-002(6图床卡·七牛展开密码框+region select)/AC-004(AppSecret 眼睛切 text) 通过、零 console error。报告 `docs/reviews/code/CODE-REVIEW-T-042-r1.md`。
...
- 下一步行动: **Sprint 4 全部任务完成 → 触发 sprint-review**。sprint-review 批量 code-review 覆盖延迟任务 **T-040/T-041/T-093 + 下载 HTML wiring 修复 + use-editor-session（auth token / security 维度）** + 完成度/范围偏移审查；输入：T-093 三观察（progress 恒0 / dragging 浮层悬停不显 / session composable security）+ T-040 wiring(导出入口/toast-host) + 下载 HTML 接线（command-registry/EditorShell）。**当前 git：分支 feature/t093-image-upload-ui，全部未提交** —— T-093 8 文件 + 下载 wiring(command-registry.ts/.test.ts、EditorShell.vue) + vite.config proxy + .gitignore(public/uploads/) + CLAUDE.md/dev-plan/EVENT-LOG；待用户决定 commit/PR。**验证栈仍在跑**：relay(bg :3000)、editor preview(:5199)、docker redis(wechat-flow-redis 既有容器)；relay 进程可 TaskStop 停。framework-feedback 已上报: [#374](https://github.com/lync-cyber/CataForge/issues/374)·penpot MCP setup 缺陷[#375](https://github.com/lync-cyber/CataForge/issues/375)·3条 upstream-gap[#376](https://github.com/lync-cyber/CataForge/issues/376)。
...
- 当前Sprint: Sprint 4（收尾）— feature 侧全完成并合 main（PR #6/#8/#10/#12/#14/#15/#16 + sprint-review 修复 PR #17 + deferred backlog [PR #18](https://github.com/lync-cyber/wechat-flow/pull/18) + post-#18 reconcile #19 + UI 轨 T-040/041/042 [PR #20](https://github.com/lync-cyber/wechat-flow/pull/20) merged）。DESIGN 门禁 **T-102/T-105/T-106 已全部 sign-off**(2026-06-25)。**全部任务完成，待 sprint-review**：T-093/T-111 + 下载 wiring 修复在 feature/t093-image-upload-ui（未提交）。下一步 sprint-review → Sprint 4 收尾。Sprint 0-3(PR #1/#2/#6/#7)。
