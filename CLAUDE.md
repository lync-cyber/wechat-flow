...
- 当前阶段: development
- 上次完成: **Sprint 6 T-071 MCP 冷启动 bench + remark pipeline 预编译单例（[PR #45](https://github.com/lync-cyber/wechat-flow/pull/45)）** —— 属 **M-009**（MCP server）。**漂移裁决**：卡片 `packages/mcp-server/`→实际 `apps/mcp-server/`、context_load `M-010/API-031`(relay/admin)→`M-009`；**AC-003/004 前提失效**（MCP server 架构不依赖 Playwright/BullMQ，长任务已经 `JobsClient` HTTP 边界解耦至 relay/job-worker，`apps/mcp-server` deps 实测无此二者）→ 标 N/A 并以架构守卫 `tests/mcp-server/cold-start-guard.test.ts`（遍历 src + package.json deps 断言无 playwright/bullmq）固化「冷路径无重依赖」精神；**用户裁决扩展 AC-005** = remark pipeline 三处理器（parse/transform/serialize）`unified()...freeze()` 模块级单例（避免 renderMarkdown 每次重建管线）。cold-start bench 复用 `tests/perf` 框架（每轮 `resetThemeRegistry` 测 `createServer→connect`，CLI 800ms 阈值 `PERF_COLDSTART_P95_MS` 可覆盖；套件内只验机制防 flake，与 typing-latency/theme-switch 一致）。**确定性兜底**：全量 vitest 173 文件/2095 绿（+15）含视觉回归 + render golden 全过 = 单例化零字节变化、经验性证伪回归担忧；pnpm typecheck 50/50、tests-tsc 0、biome 658/0；CI 5 检查（含 visual-core/sampled）全绿。new-diagnostic 假报 `coldStartLimit` 未用经 tests-tsc 0 + 眼检 line 40 证伪。**已登记 upstream-gap**（dev-plan 卡片前提与实际解耦架构矛盾）。前批 T-066 已合（[PR #44](https://github.com/lync-cyber/wechat-flow/pull/44)）。
- 下一步行动: **Sprint 6 续建中。下一批 P1 剩余 = T-087/088/089/085（T-064/066/071 已完成）→ P2（T-067/069/072）→ T-113 验证门**。下一任务 = **T-087（主题装饰资产 + SVG 注入，P1）**。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留**：T-064 `use-auto-backup` 待 wire 进 EditorShell（登记 sprint-review §wiring-completeness）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
