...
- 当前阶段: development
- 上次完成: **Sprint 6 T-069 P-004 编辑器偏好分组 + preferences-store + 接线 T-067 input-rules（[PR #54](https://github.com/lync-cyber/wechat-flow/pull/54)）** —— 属 **M-001/M-013**（F-001 AC-007 输入辅助开关 + 编辑器偏好持久化）。**产出**：新建 pinia `preferences-store`（`inputAssist`/`fontSize`/`lineHeight` + `init` + `updatePreferences`）；core `save/loadEditorPreferences`（IndexedDB `preferences` store，key `"editor"`，复用现有 schema 不改 DB_VERSION）；`EditorPreferencesSection.vue`（单 toggle + 字体 14/16/18 + 行高 紧凑/标准/宽松 1.5/1.75/2.0）；SettingsPage editor 占位符替换。**接线（props 注入范式）**：EditorShell 读 store `init()` + 下传 props → SourcePane 装 `inputRulesExtension(() => inputAssist)`（**接线 T-067 seam，toggle 非死控件**，接线测试实证 `inputAssist=true` 触发自动空格、默认 false 惰性）+ 绑 `--editor-font-size`；PreviewPane 绑 `--preview-line-height`；cm-theme fontSize 改读 `--editor-font-size`。**关键**：SourcePane/PreviewPane 保持 store-free（既有测试无 pinia），偏好一律经 props 由 EditorShell（已装 pinia）传入 = 零回归。**三处跨文档漂移裁决**：①`inputAssist` 默认值——PRD F-001 AC-007 + ui-spec P-004 **双证「默认关」** → 落 `false`，覆盖本卡 AC-002「默认 true」孤立笔误；②控件数——ui-spec 双开关 vs PRD 单开关语义 + T-067 单门控 → **单 toggle**；③字体/行高——本卡三档选择器（非 ui-spec 滑块/下拉），行高标签借 ui-spec 名称桥接值；④context_load `M-007`/`API-022` 漂移 → 持久化归 M-013 core storage（同 T-064 裁），仅留 ui-spec P-004。**兜底（主线程实证）**：62 test files / 541 tests 全绿零回归；editor+core+tests/tsconfig typecheck 0；全仓 biome 685/0。per-task code-review 延迟至 sprint-review，主线程已 inline 审全部 diff。前批 T-067 已合（[PR #53](https://github.com/lync-cyber/wechat-flow/pull/53)）。
- 下一步行动: **Sprint 6 续建中。P1 全部完成 + P2 T-067/T-069 已合流程中 → P2 余 T-072 + 就绪 T-062/063/070 → T-113 验证门**。下一任务 = **T-072（MCP Tool Schema 弃用窗口工具，属 M-009）**；另 T-062（CI 任务图完整配置）/ T-063（cross-runtime 一致性）/ T-070（版本三元组 + 确定性渲染验证）均就绪可选。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 `use-auto-backup` 待 wire 进 EditorShell（**注**：T-069 已把 `preferences-store` wire 进 EditorShell，可参照其 props/init 范式补 T-064）；② **T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟**（注入机理/PaintDrawer/测试均就绪，5 内置主题暂留 `{}` 守视觉基线，激活需配套 Linux 视觉基线重播评估）；③ **T-085 skill bundle description 自动 optimizer 未跑**（环境无头 `claude -p` 认证 401 限制，已主线程亲评优化 2 触发盲点；带 API key 或 CI 环境可补跑 run_loop 做 3-run 统计验证；skill-creator `run_loop.py` 的 Windows GBK `read_text()` 编码 bug 值得上游反馈）。**T-067 seam 已接线闭环**（T-069 已 wire `inputRulesExtension` 进 use-codemirror via SourcePane props + store 谓词）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
