...
- 当前阶段: development（Sprint 7 收尾：变体渲染缺口方案双草案已 sign-off，下一步 task-decomp 拆卡实施；T-172 r3/T-188 阶段二真机采集延后）
- 上次完成: 变体渲染缺口方案双草案 sign-off（2026-07-15）。清单轨 docs/design/VARIANT-GAP-TRIAGE-20260715-r1.md（approved；保留 83=IMPORT 47+PATCH 36/删 11/豁免 2）+ 机制轨 docs/arch/AMENDMENT-variant-mechanism-20260715-r1.md（approved；候选 B 注册期硬拒 E_VARIANT_NO_IMPL + merge 语义[块基线⊕delta，消 replace full-root 冗余] + 6 fat-base 块[callout/warning/announcement/compare/quote/pull-quote]拆最小基座+default delta + 差分守卫兜注册硬拒静态盲区[decorate no-op/delta 空] + collect-only 迁移脚手架零残留）。终审裁定 Q-B1=补差分守卫、Q-Merge=确认 fat-base 重构；顶层原则=最干净/避免向后兼容·重复包装·冗余+最佳微信兼容。前序：变体缺口清点 r1+r2（PR #121）已合 main。
  - 历史批次（残留收编 #120、红队波审闭环、PR #117 合 main 等）见 git/PR 历史与 EVENT-LOG，不在此累积。
- 下一步行动:
  - ① **变体缺口方案实施（首要，双草案已 sign-off 2026-07-15）**：已 task-decomp 拆 23 卡 T-190..T-212（dev-plan §4，cycle-free，关键路径 T-190→T-206→T-209→T-211→T-212）。**批 A 进行中：T-190（merge+fat-base）GREEN 完成、门禁全绿、待合 PR（分支 feature/variant-gap-batch-a）**。⚠️**下会话续接点**：① **T-190 REFACTOR（required，未跑）+ 审 callout `block.defaultStyle` 偏离**——callout 因 ui-spec §10.1 禁 default 变体，implementer 新增 block.defaultStyle 字段承载 default delta，引入两套 default 承载机制（5 块走 variants[] default 条目 / callout 走 block.defaultStyle）+ `defineBlock(...,undefined×3,{defaultStyle})` 尾部位参膨胀，与用户「避免冗余/最干净」原则有张力；且 callout 从不 default 渲染，其 default 字节保真守卫锁的可能是不发生的渲染路径，值得连带审 ② T-191 注册硬拒 E_VARIANT_NO_IMPL → T-192 差分守卫 → 批 B 清单（T-193..T-208）→ 批 C 收口（T-209..T-212，含全矩阵重 seed，仅 steps.card 现有变体受 merge 影响 ×5）。三批：(a) 机制批：getBlockBaseStyle/getBlockSlotStyle 改 merge（块基线⊕delta，default 降为普通变体）+ 6 fat-base 块拆最小基座+default delta（default 字节保真）+ registerBlock 注册期硬拒 E_VARIANT_NO_IMPL + 差分守卫（render(variant)≠render(default) 内部可判）+ 测试锁 variant.test.ts:84-95 重写；(b) 清单批：IMPORT 47（wechat-typeset 成建制导入）+ PATCH 36（merge delta；warning/announcement 类须含块基线键）+ DELETE 11；(c) 收口：collect-list 归零→翻 throw→移除 collect-only 脚手架→全矩阵一次重 seed（现有变体仅 steps.card 受 merge 影响 ×5；cross-runtime golden hash 硬约束不变，变红即重构 bug）。对账终裁（ARCH 层覆盖清单）：audio/video 6 变体 blocked（不计 collect-list 归零）· highlight-block.gradient feasibility 立项（拆卡前核微信真机 linear-gradient 存活）· video.autoplay DELETE。次级 OQ-2/3 taxonomy 交叠（warning↔callout↔announcement / timeline↔steps）归后续版本评估。锚点：packages/core/src/registry/variant.ts getBlockBaseStyle · packages/blocks/src/blocks/*.ts · e2e/visual/story-matrix.ts assertVariantFloor(120，落地后按最终计数重推导)
