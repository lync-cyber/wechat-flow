...
- 当前阶段: development
- 上次完成: **Sprint 6 T-070 版本三元组 + 确定性渲染验证（[PR #56](https://github.com/lync-cyber/wechat-flow/pull/56)）** —— 属 **M-002**（渲染管线 / 版本契约）。**产出**：① `packages/core/src/version/triple.ts` 追加 `getVersionTriple(themeVersion?)` —— 烘焙 `coreVersion`(core pkg) + `rulesetVersion`(ruleset pkg，均 core 合法依赖)、`themeVersion` 形参注入（默认 `0.0.0`）；② 新建 `packages/core/src/verify-determinism.ts` —— `verifyDeterminism(input, options?, iterations=3, render=renderMarkdown)`，对 `canonicalStringify({html,diagnostics})` 取 Web Crypto SHA-256，`Math.max(2,iterations)` 次比对，首次分叉即 `false`（`render` 可注入以测非确定性）；③ `apps/mcp-server/src/tools/render-markdown.ts` response 注入 `versionTriple: getVersionTriple(themeDefaultPkg.version)`（transport 层读 `@wechat-flow/theme-default/package.json` 版本，满足 AC-002 theme 来源）；④ contracts `renderMarkdownResponseSchema` 补 `versionTriple: versionTripleSchema.optional()`（向后兼容）。**关键架构裁决**：dep-cruiser `core-no-upward`+`no-circular` 禁 core 反向引用 themes（`theme-default → core` 已存在）→ core 无法直读 theme 版本，故采「core 透传 core+ruleset + transport 注入 theme」（正合任务卡 AC-001 note）；`verify-determinism.ts` 独立成文件避免 `utils/deterministic.ts`(被 inline-style 依赖)成环。**兜底（主线程实证）**：60 test files / 588 tests 全绿零回归（新增 10：version 6 + determinism 4，另 mcp/contracts 各扩 1 块）；core+contracts+mcp-server + tests/tsconfig typecheck 0；biome 171/0；模块边界 0 违规；guard:arch 0 error（2 条既有无关 no-orphans WARN 未触碰）。曾现「versionTripleSchema 未使用」IDE 滞后诊断 → 读源码(实际 tool-contracts.ts:21 已用) + tsc 0 证实假警。per-task code-review 延迟至 sprint-review，主线程已 inline 审全部 diff。前批 T-072 已合（[PR #55](https://github.com/lync-cyber/wechat-flow/pull/55)）。
- 下一步行动: **Sprint 6 续建中。P0 T-070 + P1 + P2 全部完成（T-067/T-069/T-070/T-072 已合流程中）→ 余就绪 T-062/063 → T-113 验证门**。下一任务 = **T-062（CI 任务图完整配置）/ T-063（cross-runtime 一致性 Node/Worker/Edge/Browser 四 target）** 二选一（同为 P0/就绪，无强序）；T-063 与 T-070 同族（版本/确定性属 M-002 跨运行时契约），承接顺；T-113 验证门需待其 AC 路径 forward-drift 调和。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 `use-auto-backup` 待 wire 进 EditorShell（**注**：T-069 已把 `preferences-store` wire 进 EditorShell，可参照其 props/init 范式补 T-064）；② **T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟**（注入机理/PaintDrawer/测试均就绪，5 内置主题暂留 `{}` 守视觉基线，激活需配套 Linux 视觉基线重播评估）；③ **T-085 skill bundle description 自动 optimizer 未跑**（环境无头 `claude -p` 认证 401 限制，已主线程亲评优化 2 触发盲点；带 API key 或 CI 环境可补跑 run_loop 做 3-run 统计验证；skill-creator `run_loop.py` 的 Windows GBK `read_text()` 编码 bug 值得上游反馈）。**T-067 seam 已接线闭环**（T-069 已 wire `inputRulesExtension` 进 use-codemirror via SourcePane props + store 谓词）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
