...
- 下一步行动: **architect 专项评审待启动**: CSS 内联化架构定调 — Q3.9 改写(token-driven 主动选型, 双重否决 juice/cascade-inliner: cascade 能力在 token 模型无落点 + jsdom 跨四运行时冲突) + 技术栈表/M-002§5 三处一致 amend + 样式复用模型决策(当前=主题私有副本, 无 base-style 复用层; 是否引入 block base-style + 主题 token override 两层) + 容器渲染缝隙 dev-plan 补全. **跨 Sprint deferred (1)(1b) 已收口=604a256**(zod looseObject 迁移 + juice 死依赖移除, 门禁全绿). **Sprint 4(输出能力+MCP server) 仍待用户指令启动**(post_sprint 已确认: 暂不启动). 框架反馈已上报上游 CataForge issues #234/#235/#236(详见 docs/feedback/ 及历史 commit; #236 警示: 勿用0.8.0发布版重跑 kg init --force)
...
- 待办(deferred, 跨 Sprint): (1) ✓已清=604a256 tool-contracts zod4 passthrough→looseObject(45处等价); (1b) ✓已清=604a256 移除 packages/core juice 死依赖(已被自研 styleMap 内联器取代); (2) LOW T-002 AC-003 描述过时(知会); (3) MEDIUM [T-011 R-005] postPaste true端真值路径(composeCopy stage5后调simulatePaste置true)待 Sprint 4 T-030(schema已严格化z.boolean()必填+true穿透测试已锚定, 仅剩composeCopy实现本体); (4) MEDIUM [T-010 R-001] iframe sandbox XSS阻断 happy-dom 假绿→Playwright E2E(T-058); (5) **[arch] CSS 内联化 juice 叙述 vs §5.2/§7.4 跨运行时矛盾 → 转 architect 专项评审**(代码已用自研确定性 styleMap 内联器 pipeline/inline-style.ts; 专项含 arch 三处一致 amend[技术栈表#113 + Q3.9#426 + M-002§5#modules:53] + 样式复用 base-style 两层决策); (6) **MEDIUM 容器渲染本体缝隙(S6 风险)**: ::: 容器 directive 展开 + variant 样式落地无独立任务卡(夹在 T-024 仅注册结构 与 T-087 仅上下文/装饰 之间), 与当前 transform.ts _options 未用 + inlineStyle 剥class纯tag索引 冲突; T-058 视觉回归(5主题×blocks×variants 截图)存前置依赖缺口 → S6 启动前 tech-lead 核实补卡
- 文档状态:
  - prd: approved
  - arch: approved
  - ui-spec: approved
