...
- 当前阶段: development
- 上次完成: **Sprint 6 T-066 撤销/重做 + 查找替换 + 字数统计（[PR #44](https://github.com/lync-cyber/wechat-flow/pull/44)）** —— 编辑器属 **M-001**（无 HTTP/API 契约；卡片 `M-007`/`API-006` context_load 漂移已裁决，StatusBar 实际 `components/layout/`）。撤销/重做（AC-001/002）由既有 `history()`+`historyKeymap` 满足，本任务补 `redo`/`undo` 行为 + 键位（Mod-y/Mod-Shift-z）测试；新增 `@codemirror/search` 依赖 → `find-replace.ts`（`search({top:true})`+`searchKeymap`）spread 进 `use-codemirror.ts` extensions（AC-003）；`word-count.ts`（`countWords` CJK `\p{Script=Han}` + code-point 总数 / `wordCountField` StateField，AC-006）；StatusBar metrics 契约 `{words}`→`{chineseChars,totalChars,readMinutes}`，显示 `{中文} 字 / {总} 字符`（AC-004/005），EditorShell 经 `countWords(editorStore.content)` 反应式。**主线程兜底**：IDE 诊断陈旧缓存假报 `keymap` 未导出/`findReplaceExtension` 未用/找不到模块，亲跑权威 vue-tsc+tests-tsc+biome 全 0 证伪；眼检确认 find-replace 真 spread 进 extensions（`wiring_complete=true` 属实）。vitest 170 文件/2080 绿（+18 / +3 测试文件）、editor/tests typecheck 0、全仓 biome 0。前批 T-064 已合 main（[PR #43](https://github.com/lync-cyber/wechat-flow/pull/43)）。
- 下一步行动: **Sprint 6 续建中。下一批 P1 剩余 = T-071/087/088/089/085（T-064/066 已完成）→ P2（T-067/069/072）→ T-113 验证门**。下一任务 = **T-071（MCP server 冷启动性能优化 P95<800ms，deps[T-036/037/038] 就绪）**。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留**：T-064 `use-auto-backup` 待 wire 进 EditorShell（登记 sprint-review §wiring-completeness）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
