...
- 当前阶段: development（Sprint 7：修复批一 T-160..T-174 全 DONE；修复批二+架构专项批执行中——T-175 DONE（RED 172 测试 + GREEN 定向全绿；GREEN 收尾被用户停止，全仓四门禁未收口，下一会话首要动作）、T-181 DONE（arch 0.9.0 双语义域 amendment + 45 规则归域表附录 A + 用户决策矩阵附录 B；doc-review Layer 1 分卷假阳性绕行按既有先例，Layer 2 reviewer 待跑）；**附录 B 两决策（font-family 策略/clamp 阈值冲突）待用户拍板，T-183 前置**；T-182..T-184、T-176..T-180、T-172 r3 留下一会话；PR #109 待用户审阅合并）
