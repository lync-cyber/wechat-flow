...
- 当前阶段: development
- 上次完成: **Sprint 6 T-086 编辑器性能 benchmark（feature 分支 `feature/t086-perf-benchmark`，本 PR 待合 main）** —— `tests/perf/`：`perf-runner.ts`（纯函数 computePercentiles/runBenchmark/checkThresholds/toExitCode）+ `perf-runner-cli.ts`（`pnpm bench` 入口，注册 happy-dom 全局 + 写 perf-report.json + exit code）+ `typing-latency.ts`（CodeMirror EditorView keydown→DOM 延迟）+ `theme-switch.ts`（default↔magazine 后段管线重渲，相对 import 主题注册）+ `.github/workflows/perf.yml`；新增 devDep `tsx` + `@happy-dom/global-registrator`。**AC-001 漂移裁决（PRD 权威）**：原卡测"PreviewPane DOM 更新"与 PRD §3.1 矛盾（键入延迟范围=keydown→编辑器 CodeMirror DOM，**不含 iframe 预览**），按 PRD 重锚。**阈值 env 可覆盖**（`PERF_TYPING_P95_MS`/`PERF_THEME_P95_MS` 默认 50/200）防非 M-Mac 硬件假败。vitest 36 绿、`tsc -p tests` 0、biome 0、`pnpm bench` 双向阈值实测（typing p95≈0.38ms / theme≈38ms，exit0/exit1 两分支）。perf-report.json 已 gitignore。前批 T-068 已合 main（[PR #39](https://github.com/lync-cyber/wechat-flow/pull/39)，含 #38 引入的 editor 受影响包测试 happy-dom/cwd 回归修复）。
- 下一步行动: **Sprint 6 续建中（前序"就绪队列见底"失实已纠正）。用户确认：① 继续推进就绪任务；② Playwright 本机可跑无头 → T-058 谱系解禁**。执行顺序：**下一个 = T-058（视觉回归 keystone，解锁 T-062/063/070/113）**→ P1（T-064/066/071/087/088/089/085）→ P2（T-067/069/072）→ T-113 验证门。T-058 前置已核：`playwright@^1.61.0` 在依赖树（仅需 `npx playwright install chromium`），registry `listBlocks`/`listAllVariants` 入口齐全。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
