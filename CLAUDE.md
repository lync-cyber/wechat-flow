...
- 待办(deferred)（仅列 open 项；已解决项见 git/PR 历史）:
  - **T-131 残差（B1=PR #71、B2=PR #72、[C] 交互驱动截图=本 PR 已收口）**: 余项两个 —— ① AC-004 人工视觉 sign-off：用户逐节比对 `docs/design/reports/overlay-report.html`，差异按 AC-005 转 blocking_conditions ② UC-012（通用 Modal）/UC-020（BaseColorDeriveModal）实现側为 placeholder 命令（command-registry `run:()=>{}`）无组件实体，overlay 前端侧「未生成」——属设计有/实现无的缺口，待用户裁决（补实现 or 接受为已知差异）。
