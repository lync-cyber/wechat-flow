...
- 下一步行动: **Sprint 1 已用户签字关闭**(T-098 Penpot 对照 + T-108 五AC 均确认). Sprint 1 全交付: 代码基础+设计线+UI代码(T-008~011)+sprint-review+T-108真实浏览器验证+**T-098 Penpot MCP 直接对照**(orchestrator 直连 4401/mcp 读设计稿逐项比对 P-001四档+C-001/002/004/005, DESIGN-REVIEW-s1-ui-shell-r1 approved_with_notes, DR-001 C-005 loading/error 补全). feature/sprint1-ui-shell 已合 main. **下一步进入 Sprint 2**(规则集过滤 + 诊断面板): T-013~T-019/T-094/T-052/T-099 —— 待 orchestrator 出 Sprint 2 任务编排. dev: localhost:5173. Penpot 接入须用户浏览器端连插件(manifest http://localhost:4400/manifest.json)
- 已完成阶段: [requirements, architecture, ui_design, dev_planning, cross_doc_amendment_r2]
- 当前Sprint: **Sprint 1 已关闭(用户签字)** — 代码基础/设计线/UI代码/sprint-review/T-108验证/T-098 Penpot对照 全 done, 已合 main. → **Sprint 2**(规则集过滤 + 诊断面板: T-013~T-019/T-094/T-052/T-099) 待启动
- 待办(deferred, 跨 Sprint): (1) LOW tool-contracts zod4-deprecated→S4 z.looseObject; (2) LOW T-002 AC-003 描述过时(知会); (3) MEDIUM [T-011 R-005] core RenderResult 缺 postPaste(arch M-002)→core 完善补; (4) MEDIUM [T-010 R-001] iframe sandbox XSS阻断 happy-dom 假绿→Playwright E2E(T-058); (5) **[arch] M-002 指定 juice 内联化 与 §5.2 跨运行时(browser-main)矛盾** → 已去 juice 直接构建 styleMap 解决当前; 未来复杂 CSS 主题若需浏览器 CSS inliner 由 architect 评估(amend arch M-002 或浏览器兼容 inliner)
- 文档状态:
  - prd: approved
  - arch: approved
  - ui-spec: approved
