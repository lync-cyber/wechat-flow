...
- 当前阶段: development
- 上次完成: **T-064 wire-in — `useAutoBackup` 接进 EditorShell（[PR #62](https://github.com/lync-cyber/wechat-flow/pull/62)，sprint-review §wiring-completeness 遗留①闭环）** —— composable 由 T-064（[PR #43](https://github.com/lync-cyber/wechat-flow/pull/43)）实现并单测但从未被 `EditorShell.vue` 调用（休眠死代码），自动备份功能未激活。现 `EditorShell.vue` setup 顶层调用 `useAutoBackup({ getDocId: () => editorStore.currentDocId, isDirty: () => dirtySinceBackup.value, onBackedUp: () => dirtySinceBackup.value=false })`；新增 `dirtySinceBackup` ref + `onSourceValueChange(v)` 包装器（置脏 + 转发 `editorStore.setContent`），替换 SourcePane `:on-value-change` 与 `onInsertDirective` 无 editorView 兜底分支的裸 setContent —— 实现「自上次备份以来有无变更」dirty 语义（内容变更置脏、备份成功清脏）。**未动** TopBar `:has-unsaved-changes="false"`（语义不同，避 scope creep）。**执行**：light-dispatch；3 AC 行为断言（useAutoBackup 调 1 次 + getDocId 返 currentDocId / isDirty false→true→false 状态机 / onValueChange→setContent 无回归）。**主线程独立复核**：vitest layout 目录 13 files 82 tests 全绿（含 `EditorShellDraftPersistence` 明确验 setContent→saveDraft 往返未回归）、editor vue-tsc 直跑 exit 0、biome exit 0，读 diff 确认无 scope creep + 断言行为化。前批 T-129 已合（[PR #61](https://github.com/lync-cyber/wechat-flow/pull/61)）。
- 下一步行动: **用户裁决转做非设计门禁 backlog**（设计门禁链 T-104→T-130→T-131→T-113 待 Penpot 环境就绪恢复）。剩余可离线推进的 backlog 仅 **T-128**（五主题内容元素选择器样式增强 list/ul/ol/li/img/table/a，deps[T-059]，纯 CSS 守卫增强）—— 做完 T-128 非设计门禁 backlog 即清空，届时须回到设计门禁链或由用户指定优先级。节奏：单任务一 PR、串行合并（§项目状态 单写区）。**设计门禁链恢复前置**：Penpot docker 栈起（`localhost:9001/mcp/stream` 现 HTTP 000 不可达）+ 会话重启加载 penpot MCP（.mcp.json 需重启生效）；恢复后执行顺序 = **T-104**（Penpot 深色主题 sign-off，需用户视觉）→ **T-130**（一致性工具链，依赖全部 T-095..T-106 + T-104 导全量 frame/token）→ **T-131**（视觉门禁 + 用户 sign-off，T-113 前置）→ **T-113**（Sprint 6 验证门）→ sprint-review 收尾。设计轨道 T-095..T-106 除 T-104 外 11 张稿均已 sign-off（EVENT-LOG 实证），`docs/design/` 产物固化归 T-130。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟（配套 Linux 视觉基线重播）；② T-085 skill bundle description 自动 optimizer 未跑（无头 `claude -p` 401）；③ **S6 deliverable/落点漂移聚合**（sprint-review 汇总 drift-rate，勿逐卡回改；root_cause=upstream-caused）；根因已打包上游反馈 `docs/feedback/fb-dev-plan-path-drift-writeback-20260701.md`。**T-113 forward-drift**：AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
