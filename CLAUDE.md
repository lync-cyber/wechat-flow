...
- 待办(deferred)（仅列 open 项；已解决项见 git/PR 历史）:
  ...
  - **真实环境 E2E（conditional_release，不阻塞收尾）**: T-058 linux 视觉基线播种（T-058 合并后手动跑 visual-update workflow_dispatch 在 Playwright 容器生成 linux 基线，前此 visual CI 门禁未生效）·iframe sandbox XSS（happy-dom 假绿→Playwright，可复用 T-058 Playwright 设施）·T-124 Worker delete 全局·T-125 mcp HTTP 进程·T-126 微信真实 API（需 AppID/Secret）·T-127 vite HMR 浏览器 —— 均 deferred 至部署/真实凭据环境。
