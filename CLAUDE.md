...
- 当前阶段: development
- 上次完成: **Sprint 6 T-087 主题装饰资产 + `{{tokenId}}` SVG 注入 + 上下文敏感渲染（[PR #47](https://github.com/lync-cyber/wechat-flow/pull/47)）** —— 属 **M-002 + M-005**（F-003 AC-008/009）。**调查裁决**：契约 `ThemeDefinition.assets`/`paintable` 已存在（无需新增字段、核心路径不漂移），`assets` 收窄为 `Record<string,string>`；`{{tokenId}}` 经 token registry `describeToken`（点分 `color.brand`→`#2D5A4E`，`themeOverrides[id]` 优先 `.value`）解析，非主题 `tokens` 字典。**两新 pass**：`context-aware-renderer`（祖先链 `withinBlock` → heading in-callout/standalone className，置 `inlineStyle` 后以存活 class strip）+ `decoration-injector`（`heading.{level}` SVG 经 `hast-util-from-html` 解析为 Element 注入前缀，绕开 rehype raw 不输出），render.ts 接于 inlineStyle 后/serialize 前。**确定性门控（关键）**：两 pass 仅 `effectiveTheme.assets` 非空时激活 → 空 assets/无 theme byte-identical no-op；**5 内置主题 `assets` 留 `{}` 守 Playwright 视觉基线**（Linux 基线本地不可重播），AC 由测试内带 assets 主题验证。**兜底**：全量 vitest 175 文件/2124 绿（+29）零回归（no-op 引用恒等 + 端到端 default/无 theme 渲染无装饰 HTML 双验）；pnpm typecheck 50/50、tests-tsc 0、biome 663/0；CI 5 检查（含 **visual-core/sampled 通过证门控成功**）全绿。new-diagnostic 假报 render.ts import 未用经 tsc 0 + 眼检 line 72-73 接线证伪。前批 T-071 已合（[PR #45](https://github.com/lync-cyber/wechat-flow/pull/45)）。
- 下一步行动: **Sprint 6 续建中。下一批 P1 剩余 = T-088/089/085（T-064/066/071/087 已完成）→ P2（T-067/069/072）→ T-113 验证门**。下一任务 = **T-088（编辑器 paint drawer + color picker 双向绑定 frontmatter，M-001，deps[T-029/042]）**。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 `use-auto-backup` 待 wire 进 EditorShell；② **T-087 default 主题 `assets` 激活 wiring 延迟**（注入机理 + 资产库 + 测试已就绪，5 内置主题暂留 `{}` 守视觉基线，激活需配套 Linux 视觉基线重播，paintable 字段亦待 T-088 填充）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
