...
- 当前阶段: development
- 上次完成: **Sprint 6 T-085 skill/ Skill bundle（SKILL.md + prompts + resources，[PR #52](https://github.com/lync-cyber/wechat-flow/pull/52)）** —— 属 **M-009**（F-013 AC-003 第三分发形态）。**用 skill-creator 方法论辅助设计+审查**：SKILL.md 遵循渐进披露（180 行主体，24-tool 逐项签名抽到 `skill/references/tool-catalog.md`）、命令式+解释 why（每编排步骤含"为什么"）、frontmatter description pushy 触发式。**编排链**（M-009 权威）：list_themes→describe_theme→describe_template（两阶段取数）→[register_variant]→render_markdown→simulate_paste→upload_to_wechat_asset→get_job 轮询。**AC-005 真实 dispatch**（防空心）：`createServer`+InMemoryTransport 真调 24 tool handler，仅 mock JobsClient，编排链 8 步逐步断言真实返回字段值。AC-004 真读 skill/ 与 mcp-server 两 package.json 版本对齐（均 0.0.0）。resources 用真实主题 id（tech/literary）+ 真 directive。**触发优化**：自动 optimizer（run_loop→`claude -p`）因**无头认证 401** 环境限制不可用（另暴露 skill-creator 脚本 Windows GBK 编码 bug + WinError 10038 socket 并行 bug），降级为**主线程亲评**（orchestrator 即目标模型）逐条 20 eval，修 2 触发盲点：①「写成」歧义→「已写好的 Markdown」+「不代写内容」防内容创作误触发；②补「导出长图/封面」防漏触发；末尾排除 Notion/邮件/小程序/网页非公众号场景。**兜底**：tests/skill 18/18 绿（真实 dispatch）；tests-tsc 0；biome 676/0；纯新增未碰现有源码=零回归。前批 T-089 已合（[PR #51](https://github.com/lync-cyber/wechat-flow/pull/51)）。
- 下一步行动: **Sprint 6 续建中。P1 全部完成（T-064/066/071/085/087/088/089）→ 进 P2（T-067/069/072）→ T-113 验证门**。下一任务 = **T-067（输入辅助：自动空格 + 智能引号，F-001 AC-007，基于 CodeMirror 6 InputRule）**。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 `use-auto-backup` 待 wire 进 EditorShell；② **T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟**（注入机理/PaintDrawer/测试均就绪，5 内置主题暂留 `{}` 守视觉基线，激活需配套 Linux 视觉基线重播评估）；③ **T-085 skill bundle description 自动 optimizer 未跑**（环境无头 `claude -p` 认证 401 限制，已主线程亲评优化 2 触发盲点；带 API key 或 CI 环境可补跑 run_loop 做 3-run 统计验证；skill-creator `run_loop.py` 的 Windows GBK `read_text()` 编码 bug 值得上游反馈）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
