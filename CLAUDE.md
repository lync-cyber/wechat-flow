...
- 当前阶段: development
- 上次完成: **Sprint 6 T-072 MCP Tool Schema 弃用窗口工具（[PR #55](https://github.com/lync-cyber/wechat-flow/pull/55)）** —— 属 **M-009**（MCP server 基础设施）。**产出**：`apps/mcp-server/src/deprecation.ts` —— `markDeprecated(toolName,field,since,until)` 注册到内存 `Map`；`checkDeprecations(now?)` 返回 `DeprecationWarning[]`（`expired = now 严格大于 until`，`now` 可注入默认 `new Date()` 以确定性测试）；`clearDeprecations()` 供测试隔离。`createServer`（stdio.ts）启动时对 `expired` 条目 `console.warn`（默认空注册表 → 零启动噪音，既有 137 测试零回归）。当前无实际弃用项，本任务为未来字段/Tool 弃用平滑迁移的基础设施。**漂移裁决**：①deliverable 路径——任务卡 `packages/mcp-server/...` 漂移 → 实际 `apps/mcp-server`（M-009），测试落根 `tests/mcp-server/`（既有约定，相对路径 import 源码，无 co-located `__tests__/`）；②context_load `M-010`（中继客户端）/`API-031` 弱相关 → 归 M-009。**兜底（主线程实证）**：18 test files / 137 tests 全绿零回归（7 新增，含 AC-004 console.warn spy + 严格边界）；mcp-server + tests/tsconfig typecheck 0；全仓 biome 687/0。曾现「checkDeprecations 未使用」滞后诊断 → 主线程读源码 + tsc 证实为 IDE 缓存假警（接线 loop 实际存在）。per-task code-review 延迟至 sprint-review，主线程已 inline 审全部 diff。前批 T-069 已合（[PR #54](https://github.com/lync-cyber/wechat-flow/pull/54)）。
- 下一步行动: **Sprint 6 续建中。P1 全部完成 + P2 全部完成（T-067/T-069/T-072 已合流程中）→ 余就绪 T-062/063/070 → T-113 验证门**。下一任务 = **T-070（版本三元组 + 确定性渲染验证，P0，M-002）** 建议优先（P0 优先级最高）；另 T-062（CI 任务图完整配置）/ T-063（cross-runtime 一致性 Node/Worker/Edge/Browser 四 target）就绪可选。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 `use-auto-backup` 待 wire 进 EditorShell（**注**：T-069 已把 `preferences-store` wire 进 EditorShell，可参照其 props/init 范式补 T-064）；② **T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟**（注入机理/PaintDrawer/测试均就绪，5 内置主题暂留 `{}` 守视觉基线，激活需配套 Linux 视觉基线重播评估）；③ **T-085 skill bundle description 自动 optimizer 未跑**（环境无头 `claude -p` 认证 401 限制，已主线程亲评优化 2 触发盲点；带 API key 或 CI 环境可补跑 run_loop 做 3-run 统计验证；skill-creator `run_loop.py` 的 Windows GBK `read_text()` 编码 bug 值得上游反馈）。**T-067 seam 已接线闭环**（T-069 已 wire `inputRulesExtension` 进 use-codemirror via SourcePane props + store 谓词）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
