...
- 当前阶段: development
- 上次完成: **Sprint 6 T-067 输入辅助 自动空格+智能引号+破折号（[PR #53](https://github.com/lync-cyber/wechat-flow/pull/53)）** —— 属 **M-008/M-001**（F-001 AC-007 实时输入辅助）。**机制**：CM6 无内置 InputRule，用 `EditorState.transactionFilter` 拦截 `input.type` **单字符**插入（PRD「单字符级生效」）。**双层**：`matchInputRule(before,inserted)` 纯函数（六规则核心，脱 EditorView 可测）+ `inputRulesExtension(enabled?)` 工厂（transactionFilter + 单字符 guard + `enabled` 谓词门控，供 use-codemirror `extraExtensions` 装配，本任务不接线）。**规则对齐核心 zh-typo**：中英**双向**空格（对齐 `zh-en-space`）、破折号 `--`→`——`（对齐 `ellipsis-dash`）；智能引号用**位置启发式**（行首/空格后 open `“‘`，其他 close `”’`），与修订流配对式正交。**PRD↔dev-plan 漂移处置**：①**破折号缺失**（PRD AC-007 列三项，原卡漏破折号，主任务表亦漏）→ 用户授权本轮扩入 AC-006 并同步任务卡；②**默认值冲突**（PRD「默认关」vs T-069「inputAssist 默认 true」）→ 按 PRD 权威登记，T-067 仅建 `enabled` seam，产品默认值由 **T-069** store 接线落 `false`；③测试路径随 `extensions/__tests__/` 既有约定（非卡片旧写法）。**与 use-zh-typo 正交**：后者是粘贴/手动修订（预览→全文替换），本任务是输入时改写，不重叠触发。**兜底**：单测 29/29 绿（纯函数逐 AC + 集成 + 粘贴/IME/删除三重防护，主线程实证）；editor vue-tsc 0；全仓 biome 678/0；纯新增扩展模块未碰现有源码=零回归。per-task code-review 按框架延迟至 sprint-review 批量（非高风险任务），主线程已完成 inline 审查。前批 T-085 已合（[PR #52](https://github.com/lync-cyber/wechat-flow/pull/52)）。
- 下一步行动: **Sprint 6 续建中。P1 全部完成 + P2 T-067 已合流程中 → P2 余 T-069/072 → T-113 验证门**。下一任务 = **T-069（P-004 设置页「编辑器偏好」分组：输入辅助 toggle + 字体大小 + 行高，deps[T-042,T-066,T-067] 已全就绪）**。⚠️ **T-069 必须消化 T-067 遗留的默认值裁决**：新建 `preferences-store` 的 `inputAssist` 默认值——PRD F-001 AC-007「默认关」为权威，与 T-069 卡 AC-002「默认 true」冲突，**T-069 应落 `false`（默认关）**并把 store 谓词 `() => preferences.inputAssist` 接线进 `use-codemirror` 的 `inputRulesExtension(enabled)`（T-067 已留 seam，本身不接线）。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 `use-auto-backup` 待 wire 进 EditorShell；④ **T-067 `inputRulesExtension` 待 T-069 wire 进 use-codemirror + store 谓词接线**（T-067 纯建 seam 不接线）；② **T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟**（注入机理/PaintDrawer/测试均就绪，5 内置主题暂留 `{}` 守视觉基线，激活需配套 Linux 视觉基线重播评估）；③ **T-085 skill bundle description 自动 optimizer 未跑**（环境无头 `claude -p` 认证 401 限制，已主线程亲评优化 2 触发盲点；带 API key 或 CI 环境可补跑 run_loop 做 3-run 统计验证；skill-creator `run_loop.py` 的 Windows GBK `read_text()` 编码 bug 值得上游反馈）。**T-113 验证门 forward-drift**：其 AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
