...
- 当前阶段: development
- 上次完成: **Sprint 6 dev-plan amendment — 补 T-130/T-131「前端 vs Penpot 设计稿视觉一致性门禁」（[PR #60](https://github.com/lync-cyber/wechat-flow/pull/60)）** —— 纠正设计→实现顺序倒置（项目先实现前端、后补 Penpot 设计，违背 design→impl 依赖），补系统化视觉门禁。**用户三裁决**：半自动混合（token 硬门禁 + Playwright 并排差异报告 + 人工 sign-off）× 全量 UC-001..023 + P-001..005 × Sprint 6 阻塞门。**产出**：① **T-130**（feature/standard，P0/large，deps[T-058,T-059,T-095..T-106]）设计一致性工具链 —— `scripts/design-sync/{export-penpot-tokens,export-penpot-frames,token-diff,build-overlay-report}.ts` + `docs/design/{tokens,frames,reports}/` 固化产物 + `e2e/visual/design-overlay.spec.ts`（新 Playwright project 独立 webServer 起真实编辑器 SPA，区别 T-058 story-render 自一致性基线）；② **T-131**（validation/skip，P0，user_facing_critical_path，deps[T-130,T-104]）门禁 —— `pnpm design:token-diff` exit-code 硬校验（不一致即卡 DONE，禁 [ASSUMPTION]/[ENV-LIMITATION] 豁免）+ 并排报告 + 用户视觉 sign-off，残差差异走 `conditional_release.blocking_conditions` 逐条闭环；③ **T-113** deps += T-131 + 新增首条 AC「设计一致性门禁通过」+ 通过条件 13→14；[NAV] 追加 T-130/T-131。**关键裁决**：(a) tech-lead 纠正既有 T-098/T-101 卡 schema 不合法 `event=design_signoff` → 合法 `event=user_decision`（design_signoff 语义入 detail；实证 22 条历史 signoff 均 user_decision）；(b) UC 计数纠偏 —— 分卷文件名 `uc001-uc014` 实含 UC-001..023；(c)「设计产物固化」（Penpot frame/token 导出入 git）为 T-130 一等 deliverable 非假设已存在（`docs/design/` 原不存在）。**兜底**：doc-review Layer 1 净零 self-caused（vs HEAD 基线 4 FAIL 逐字一致；T-130 漏 `tdd_acceptance` 已补 `all`；残留 PS-007/008 交叉引用 + deliverables/tdd_acceptance 各缺是 approved 文档既有项，不做 scope creep）；主线程等价 Layer 2 内联审全部新卡。前批 T-062 已合（[PR #59](https://github.com/lync-cyber/wechat-flow/pull/59)）。
- 下一步行动: **Sprint 6 视觉门禁链已入计划，执行顺序纠正为 design→gate**。未实现执行顺序 = **T-104**（Penpot 深色主题设计 sign-off，需用户视觉）→ **T-130**（设计一致性工具链，TDD standard；依赖全部 Penpot 设计轨道 T-095..T-106 + T-104 方能导出全量 frame/token，某 UC 稿缺失时 AC-002 列缺失清单阻断）→ **T-131**（视觉门禁 + 用户 sign-off，T-113 前置）→ **T-113**（Sprint 6 验证门）→ sprint-review 收尾（聚合 §漂移 ④ + wiring-completeness）。节奏：单任务一 PR、串行合并（§项目状态 单写区）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-064 `use-auto-backup` 待 wire 进 EditorShell（参 T-069 preferences-store 范式）；② T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟（配套 Linux 视觉基线重播）；③ T-085 skill bundle description 自动 optimizer 未跑（无头 `claude -p` 401）；④ **S6 deliverable/落点漂移聚合**（sprint-review 汇总 drift-rate，勿逐卡回改；root_cause=upstream-caused，代码/功能均正确）；根因已打包上游反馈 `docs/feedback/fb-dev-plan-path-drift-writeback-20260701.md`。**T-113 forward-drift**：AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
