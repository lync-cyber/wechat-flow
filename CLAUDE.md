...
- 当前阶段: development
- 上次完成: **跨运行时 markdown 加载 blocker 修复 + T-040/T-041 GREEN 完成并浏览器实测（2026-06-25，分支 feature/t040-job-progress-toast，未提交）** —— blocker：5 主题包 `templates/index.ts` 在模块加载期 `node:fs.readFileSync` 读 starter.md，浏览器无 node:fs → editor 在 `app.mount` 前崩、`/` 与 `/themes` 空白（T-092/PR #16 引入，被 jsdom/mock 假绿掩盖）。修复方向（用户定 codegen 单源）：`.md` 仍为单源 → `scripts/gen-theme-templates.mjs`（根 `pnpm gen:templates`）glob `packages/themes/*/templates/*.md` 生成提交的同包 `src/templates/<base>.generated.ts`（`JSON.stringify` 安全转义反引号/代码围栏），`index.ts` 改 import 生成模块去 fs；新鲜度由 **pre-push 守护**（`gen-templates-fresh`：重生成后 `git diff --exit-code` 生成物，漂移即拦 push，race-free）；生成物加入 biome `files.ignore`（产物不被 formatter 加工，保 gen 输出与提交版字节一致）。core AC-005 守护仍 fs 读 `.md` fixture（Node 环境）不变、与运行时同源不漂移。验证：typecheck 48/48、相关单测 31 passed（core 主题守护 23 含 AC-005 + ThemesPage 8）、editor 浏览器 `/`+`/themes` 实测挂载（#app 有内容 / 5 卡 / 5 缩略图 / 5 主题名 / 零 console error）。T-040(JobProgressBar+Toast+use-sse-job)、T-041(/themes 主题市场页+UC-022 卡) 已 commit（e100b64/2ef1d97）。此前 DESIGN 轨三批 T-102/T-105/T-106 Penpot 视觉稿全部 design_signoff 通过（详见下），绘于文件 "wechat-flow Design System" 新页 "S4 — 组件视觉稿"（12 board）。
...
- 下一步行动: **UI 实现轨 — T-040/T-041 已完成, blocker 已清, 余 T-042/T-093 待 TDD**。① 决策点：blocker 修复（6 文件改 + 6 新增）当前在 feature/t040-job-progress-toast 工作区未提交，待用户确认是否提交。② **T-042** P-004 设置页 /settings 图床+API密钥分组(deps T-005,T-033,T-102，security_sensitive=true → light-dispatch 而非 inline) → **T-093** UC-018 编辑器内上传 UI 接线 拖拽/粘贴/进度/重试(deps T-033,T-091,T-106)。二者 apps/editor Vue 实现, 走 standard TDD(tdd-engine)，实现后 preview_* 浏览器验证。两者 done 后 → **T-111 [VALIDATION]**(集成验收, 含主题模板市场卡缩略图 pixelmatch ratio≤0.05 对照 T-105 基准 + UI 三档响应式) → Sprint 4 收尾 → sprint-review。遗留 MEDIUM：T-040 组件 wiring_complete=false（待导出入口/toast-host 挂载）。HEAD=#19(post-#18 reconcile chore)。framework-feedback 已上报: [#374](https://github.com/lync-cyber/CataForge/issues/374)·penpot MCP setup 缺陷[#375](https://github.com/lync-cyber/CataForge/issues/375)·3条 upstream-gap[#376](https://github.com/lync-cyber/CataForge/issues/376)。housekeeping(待清): 本地 chore/post-merge-reconcile-pr18 + origin 三孤儿分支(chore/post-merge-reconcile-pr18·chore/relay-post-merge-reconcile·feature/relay-backend, 均已合并)。
