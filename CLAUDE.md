...
- 当前阶段: development
- 上次完成: **dev-plan s4 amendment** — T-030/031 deliverable 路径 `packages/core/src/composers/`→**`apps/editor/src/use-cases/`**（s4 frontmatter 0.5.0→0.5.1，索引重建 69 文档，doctor 全绿）。依据：M-008 是触碰 `navigator.clipboard` 的应用层 use case，落 apps/editor 而非 framework-agnostic packages/core 才架构正确（承接 s1 render.ts 同款 ARCH §7.2 目录决策伏笔）；测试路径 `tests/app-layer/` 未漂移，dual-mime 实际落 use-cases/（非 dev-plan 原写的 clipboard/）。**已 commit 到本地分支 `fix/dev-plan-s4-use-cases-path`（309037f；未推送、未开 PR，待与后续切片一并推 main）**。前序：framework scaffold 同步 0.12.0→0.13.0；输出能力切片 **T-030 composeCopy + T-031 composeExportHtml done，PR #8 已合 main**（composeRender→simulatePaste→buildDualMimePayload→clipboard.write + Toast + AC-004 接线 EditorShell→TopBar `:on-copy`）；L3 cascade T-118→T-121（PR #6）/ framework 0.12.0（PR #7）。运行学习：①接线类 AC 须行为级测试 ②standard 跨包卡子代理易 truncation→主线程接管 ③并行子代理勿各自跑 git stash（竞态污染工作树/诊断），不轻信自报告、主线程取地面真值复核 ④白名单 SSOT=packages/core/src/registry/css-property-whitelist.ts ⑤`cataforge deploy --rebuild` 会删 CLAUDE.md 致 §项目状态 静默 wipe；刷产物用普通 deploy，误删则 git restore + 普通 deploy 恢复。
- 下一步行动: dev-plan 漂移已清并 commit 到本地分支 `fix/dev-plan-s4-use-cases-path`（未推送）。Sprint 4 待选切片（用户选）：MCP server(T-036→T-037/038→T-122)、relay 后端(T-032..035/091)、编辑器 UI(T-040/041/042/093)、主题 template T-092、DESIGN(T-102/105/106)、VALIDATION T-111。注：s5 的 apply-zh-typo/upload-wechat-asset 仍写 composers/ 属未来卡，由 implementer 落地时按 ARCH §7.2 决策（未来 use case 落点统一倾向 apps/editor/src/use-cases/），非本次漂移范围。
