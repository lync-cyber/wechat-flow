...
- 当前阶段: development
- 上次完成: **Sprint 6 T-060 已知 Bug 补丁库热加载（feature 分支 `feature/t060-patch-hotload`，待本 PR 合 main）** —— `loadPatchBundle(url)` = fetch + JSON 解析 + 结构校验（version 为 string、patches 为数组、每条 id/scope/priority），四类失败（网络异常 / `!response.ok` / JSON 解析 / schema）统一抛 `PatchLoadError`；`applyPatchBundle` 全量校验通过后才注入（原子性，任一非法整体 reject 不污染已有规则），经 registry 新增 `upsertRule`（findIndex 替换 / 否则 push）实现同 `rule.id` 覆盖（热修复语义，非追加）；`version` 语义 = 微信客户端版本号（ARCH M-003）。17 tests（AC-001×2 / 002×3 / 003×9 / 004×3），`tests/ruleset/` 整目录 184 passed / 11 files 全绿，tests typecheck（`tsc -p tests/tsconfig.json`）exit 0、biome 0。GREEN implementer 仍漏跑 tests typecheck（同 T-061 失败模式），主线程兜住并清理测试残留（死变量 rulesBefore/bundlePayload/fakeBundle + 含自述注释的 no-op afterEach，违 COMMON-RULES §禁止设计阶段残留）。任务卡 3 处漂移已纠正回写：`listRules()`→`getRules()`、测试目录 `src/__tests__/`→根 `tests/ruleset/`、`arch#§3.API-011`（实为 describe_variant）失效引用 → 权威取 PRD F-011 AC-005 + ARCH M-003。前批 T-057 / T-061 已合 main（[PR #34](https://github.com/lync-cyber/wechat-flow/pull/34) + relay e2e flaky 修复 [PR #35](https://github.com/lync-cyber/wechat-flow/pull/35)）。
- 下一步行动: **Sprint 6 进行中 —— T-060 后剩余就绪任务 T-090（实地验证辅助脚本 + EVENT-LOG 回写，P1，deps[T-057]✅）**。T-062（deps T-058）/ T-063（deps T-058）/ T-070（deps T-063→T-058）仍阻塞于 **T-058（Playwright 视觉回归，环境受限）**；T-113 验证任务待全部质量门任务收口。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
...
- 当前Sprint: **Sprint 6（质量门 + 视觉回归 + 性能基准 + skill bundle）；已合 main: T-056 / T-059（[PR #32](https://github.com/lync-cyber/wechat-flow/pull/32)）+ T-057 / T-061（[PR #34](https://github.com/lync-cyber/wechat-flow/pull/34)）；本 PR 待合: T-060**。Sprint 0-5 全部合 main（PR #1~#31）；逐 sprint 历史见各 dev-plan、EVENT-LOG 与 PR 记录，不在本状态区复述。
