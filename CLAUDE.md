...
- 当前阶段: development（Sprint 7 收尾：变体缺口方案批 A+批 B 全部完成，批 C 收口卡用户门卡 + 技术前置）
- 上次完成: **变体缺口方案批 B（清单批）全部完成（2026-07-16，PR 待合，分支 feature/variant-gap-batch-b）**——96 项裁定全落地：IMPORT/PATCH 83 项 across T-193..T-205（table/compare/quote/card/author-card/steps/pull-quote/highlight-block/warning/disclaimer/citation/reading-time/kpi-card/qa/tip-grid/definition-list/footnote/heading/paragraph/list/divider/code-block/image/image-caption/dialog/timeline/qrcode/miniprogram-card/footer-cta/recommendation/related-cards/social-cta/subscribe-cta/advert-card）+ DELETE 10 项（T-208）+ audio/video 3 项 contract-pending 归桶（T-207，known-blocked-variants.ts）。装饰节点统一确立正确范式：真实 slot + 主题 token（quote 整改删除空串 data-block-slot hack + 硬编码色）。收波修复：em-to-px matcher /g flag flaky（独立 fix commit）· transform-directive AC-004 代表变体（compare/dialog 消费属性后改 default）· T-207 归桶 fallout（T-191/T-192 守卫测试断言改 known-blocked 排除验证）· p1-incremental/p1-essentials 配额对齐 sign-off 后真实契约（quote≥9/card≥4/publication-skeleton 单变体块）。code-block.light 给 5 主题各补浅色 code token。5 波 commit 全绿（各波全量 vitest+tests tsc 直跑+typecheck+biome+cross-runtime+stash 空）。前序：批 A 机制批 T-190（#125+#127）· T-191（#128）· T-192（#129）均已处理。
  - 历史批次见 git/PR 历史与 EVENT-LOG，不在此累积。
- 下一步行动:
  - ① **批 C 收口（T-209..T-212）—— 卡三类前置（首要）**：批 A+批 B 已交付，批 C 翻转动作（T-209 registerBlock 翻 throw + 移 collect-only 脚手架 · T-210 差分守卫 WARN→RED · T-211 全矩阵重 seed+assertVariantFloor 重推导 · T-212 用户走查）**均要求归零对账为空集**（T-209/T-210 AC-001 明确禁放宽豁免掩盖遗漏）。orchestrator 预检（临时脚本跑 runVariantDiffGuard + getUnimplementedVariants 扣 known-blocked/allowlist）暴露三类前置阻塞：
    - **(a) 用户门卡 [owner=user]**：`highlight-block.gradient` 真机核实（T-206，无自动 oracle，dev-plan 建议与 T-172 r3 真机采集合并；真机不存活→DELETE，默认亦 DELETE）——未处理则 T-209/T-210 归零永非空。
    - **(b) T-191 谓词覆盖缺口 [owner=architect]**：① decorate 块级粒度——块有 decorate 即放行全部具名变体（谓词②），不判该 decorate 是否真处理该变体（quote 等）；② **块外机制实现的变体假阳性**——`divider.{wave,dots,flower}` 经 ruleset/mark 注入 SVG（渲染≠default 真有实现，已核实渲染产物含 svg path/circle/line），但无 baseStyle 无 block-decorate，被 getUnimplementedVariants 假阳性 flag。T-209 翻 throw 前须 architect 裁定如何正确识别这类 externally-implemented 变体（第四谓词 or 专门登记，非掩盖遗漏 / 非塞 plain-allowlist）。
    - **(c) 差分守卫 fixture 覆盖 [T-210 技术前置]**：`list.{bullet,numbered,checklist}`/`code-block.{light,minimal}`/`gallery.{duo,grid}` 在全局差分守卫下假 finding——根因通用 fixture（DEFAULT_DIRECTIVE_BODY 一段文字）对需结构化内容的块（列表/代码/图片）不匹配，decorate 找不到 ul/pre/img 不触发装饰致渲染≡default。各卡测试当初用局部自定义 buildMarkdown 绕过，T-210 翻 RED 前须给差分守卫内容感知 fixture（列表块喂列表 markdown、code-block 喂代码块、gallery 喂图片列表）。
    - 执行顺序建议：先 (b)(c) 技术前置（不依赖用户）→ 待 (a) gradient 用户裁定 → T-209/T-210 归零翻转 → T-211 重 seed → T-212 走查。
  - 独立分流：tip-grid/definition-list（及 steps/list/recommendation）列表 decorate 工具 ~60 行重复 → 提取 decorate-utils.ts（已 spawn_task task_6198f478，out-of-scope 优化，不阻塞批 C）。
