...
- 下一步行动: 链条 `T-104✓→T-130✓→T-131✓→T-113`。**T-131 conditional_release 的 3 条 blocking_conditions 已按用户决策 disposition 至 §待办(deferred)（核实后全部 backlog、后续会话择机修复）→ `blocking_conditions` 清空 → Sprint 6 DONE 解阻**。B1 已核实 = 桌面侧栏文档列表**未实现**（`LeftPanelTabs.vue` docs Tab 为占位 stub `docs-content`；移动 `DocumentListSheet.vue` 已实现）；B2=fixture 未 seed(LOW)；C=overlay 方法局限（设计板已 sign-off、实现各卡已验证）——均转 backlog。**下一卡 = T-113**（Sprint 6 验证门，14 项）→ sprint-review 收尾。节奏：单卡一 PR、串行合并（§项目状态 单写区）。设计轨道 T-095..T-106 全 sign-off（含 T-104）。设计轨道 T-095..T-106 全 sign-off（含 T-104）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟（配套 Linux 视觉基线重播）；② T-085 skill bundle description 自动 optimizer 未跑（无头 `claude -p` 401）；③ **S6 deliverable/落点漂移聚合**（sprint-review 汇总 drift-rate，勿逐卡回改；root_cause=upstream-caused）；根因已打包上游反馈 `docs/feedback/fb-dev-plan-path-drift-writeback-20260701.md`。**T-113 forward-drift**：AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
