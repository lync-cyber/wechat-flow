...
- 当前阶段: development（Sprint 7：修复批一 T-160..T-174 全 DONE；修复批二+架构专项批执行中——T-175 DONE 且全仓四门禁已收口（vitest 全仓 3473 PASS/0 FAIL/29 skip、turbo typecheck --force 50/50、tests-tsc、biome 848 全绿；收口发现 05-block-directive-callout e2e 字节快照因 div→section 容器原语变更 stale，已 regenerate commit f32136f 单点绿）、T-181 DONE（arch 0.9.0 双语义域 amendment + 45 规则归域表附录 A + 用户决策矩阵附录 B；doc-review Layer 1 分卷假阳性绕行按既有先例，Layer 2 reviewer 待跑）；**附录 B 两决策（font-family 策略/clamp 阈值冲突）待用户拍板，T-183 前置**；T-182..T-184、T-176..T-180、T-172 r3 留下一会话；PR #109 待用户审阅合并）
