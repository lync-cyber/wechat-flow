...
- 上次完成: **设计一致性收敛 backlog developer MEDIUM 批 7 项收敛（PR #96，堆叠于 #95）**：BC-2 图标体系单色收敛（Toast 类型图标/DropdownMenu+ContextMenu 前置图标去重复分隔线/BlockLibItem blockGlyph+皮肤 pill/ImageUploadOverlay 拖拽下箭头+idle 去彩 emoji，全部对齐设计帧单色 glyph 而非彩色 emoji——纠正侦察代理误判）、BC-3 DiagnosticsItem 补「查看」链接（EditorShell onDiagnosticView 定位预览节点瞬时高亮；nodeRef 为裸 tagName 故精确源行跳转俟上游）、BC-5 顶栏图标钮 ghost 化（+/□/... 此前裸 button 出 UA outset 边框，补 top-bar__icon-btn 透明无边框，对齐 UC-003）、BC-7 ThemeCard 缩略图填主题 brand 色块 + 12px 描述副行（themeMetaSchema 增 description 并登记 5 主题，宿主经 describeTheme 传真实 tokens）、BC-8 TemplateThemeCard 缩略图吃主题 --color-background/surface/border token（tech 暗色 #0F1117 实证得暗底）、BC-9 SettingsPage 简化顶栏（← 返回编辑器 → / + 「设置」标题）、BC-10 预览页去视口切换 tabs（PreviewPane showToolbar 开关）+ 缺失文档回退 demo 内容（loadDraft 不再空白）。浏览器实链逐项验证（菜单单色图标/toast 红✕/BlockLibItem glyph+pill/顶栏钮 border none/themes 页 tech 暗底缩略图/settings 返回导航/preview 无 tabs+正文 2862 字符）+ 全仓门禁绿（vitest 2817/1 skip、editor vue-tsc、tests tsconfig 50 包、biome）。**Phase 1-7 + post 全部完成。**
- 下一步行动: **项目 completed，剩余均为用户侧决策**（upstream 三件套已提报 #421/#422/#423，见 §待办）：① release go/no-go——包版本核对（现全 0.0.0）、mcp-server 是否纳入发布面（现 private:true，翻转后 `npx @wechat-flow/mcp-server` 才真正可用，发布集 13→14 需同步 deploy-spec §9-D1）、GitHub Environment npm-publish required-reviewers 配置、PRD §3.5 真实公众号粘贴回归、Docker 环境容器 bring-up 实测、CVE 门禁阈值（deploy-spec §9-D4 needs_input）② **CataForge 0.15.0 升级可用**（本地 editable 包已 0.15.0、scaffold 仍 0.14.0，doctor 报 context.mode hybrid 枚举失效 + 5 处引用逃逸 FAIL 均属升级漂移）——运行 framework-update skill 对齐四层后再做后续会话 ③ **设计一致性收敛 backlog**（T-131 AC-004 二轮 sign-off 产出 15 条 BC，两 HIGH（BC-1/BC-11）+ developer MEDIUM 批 7 项（PR #96）已收敛，余 6 条 open，见 §待办 + SPRINT-REVIEW-s6-r3.md）——developer 待决 2 条：BC-4 状态栏三指标（甄别=未实现，功能缺口，需 spec 确认可读性算法）、BC-6 修订预览信息架构（明确 A/B 裁决项）；BC-12/13 已裁转设计侧并入 BC-15，UC-014 设计帧重导出 + BC-15 设计侧对齐随 amendment 批走；其余 dev 残留（T-033 env-gated、R-007、T-124/T-126 需真实凭据、strip 双规则独立会话）见 §待办 ④ arch 措辞修订三项已登记 §待办，随 framework-update 后的 amendment 批走。
...
- 待办(deferred)（仅列 open 项；已解决项见 git/PR 历史）:
  - **设计一致性收敛 backlog（T-131 AC-004 二轮 sign-off 产出，verdict=conditional_release，逐条见 `docs/reviews/sprint/SPRINT-REVIEW-s6-r3.md`）**: 15 条 blocking_conditions 已 disposition 至此 backlog（同上轮 r1 模式，清空解阻，不回退 Sprint 6 DONE / 项目 completed）；BC-1（HIGH，UC-019 PaintDrawer，PR #94）、BC-11（HIGH，UC-021 二段式，PR #95）与 developer MEDIUM 批 7 项（BC-2/3/5/7/8/9/10，PR #96）已收敛，余 6 条 open。**developer MEDIUM 已收敛（PR #96）**：BC-2 图标体系单色收敛(Toast/菜单/BlockLibItem glyph+pill/上传浮层拖拽箭头)、BC-3 UC-013 issue 行「查看」链接(nodeRef 裸 tagName 故预览节点最佳努力高亮)、BC-5 UC-003 顶栏图标钮 ghost 化(裸 button UA 边框→透明无边框)、BC-7 UC-007 ThemeCard brand 缩略图+12px 描述副行(themeMetaSchema 增 description)、BC-8 P-003 TemplateThemeCard 缩略图吃主题 background token(tech 暗底实证)、BC-9 P-004 SettingsPage 简化顶栏(← 返回编辑器)、BC-10 P-005 去视口 tabs(PreviewPane showToolbar)+缺失文档回退 demo。**developer 待决（2 条 open）**：BC-4 UC-023 状态栏指标段——甄别结论=未实现（可读性无数据源/keyword-lint 与 nightRisk 存在但未接线），属功能缺口非视觉 polish，需确认 UC-023 是否定义可读性算法后再定；BC-6 UC-017 修订预览信息架构——明确裁决项（改前端双栏重构 vs 反向登记设计侧，前端现为规则 ID 列+行内 diff）；**BC-11 已收敛（HIGH，PR #95）**：UC-021 二段式流程为契约必选——二级 variant 面板+参数表单（超 3 字段跳 InsertDrawer）+插入后段 + 分类 tab/variant 计数角标/行首图标（block-glyphs）+ snippet class 语法 `:::block{.variant}` / inline 参数外置 `:mark[]{k=v}`，浏览器实链验证全通。**BC-12/BC-13 已裁决反向登记设计侧**：BC-12 UC-020 派生分组（token 名标注+数据驱动分组为可维护性更优实现）、BC-13 UC-022 selected 满铺 brand-subtle（有意增强保留），并入 BC-15 批同步 Penpot，owner=ui-designer。**ui-designer**：BC-14 UC-014 Penpot 空帧重导出(MEDIUM)、BC-15 设计侧对齐批(LOW，kbd 徽章 UC-009/010/012/016 / UC-005 切换器 / UC-015 分类 tab / UC-009 Ctrl+\ stale / UC-006 主题卡 / P-003 条色 / UC-001 文案，随 arch amendment 批同步 Penpot)。判一致 4 节(UC-004/UC-010/UC-012/P-001)+LOW 散点接受偏差见 r3。
