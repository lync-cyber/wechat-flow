...
- 下一步行动: **Sprint 5 进行中 — 并行实现批 T-047/T-051/T-073/T-074/T-077 已完成，在 feature 分支 `feature/sprint5-impl-batch` 待 PR 合 main；T-081~T-084 已直提 main（`95e8716`/`c5f8438`，已 push origin/main）**。本批全绿（全量 116 files/1379 passed/1 skip/0 fail、全 typecheck 0、biome 0）：T-047 插件沙箱骨架(plugin-api,25 测试,standard/security) · T-051 HTTP transport + admin API key(security；code-review **needs_revision 2 HIGH+4 MEDIUM+3 LOW，用户选全修已闭环**：admin-guard loopback 缺省白名单 + Bearer scope 校验 / HMAC+PEPPER 哈希 / actor=apiKeyId / PATCH+DELETE 端点 / XFF 右解析 / 空 body 测试 / store wiring 标注) · T-073 themes 市场增强(筛选 + 5 主题各 ≥2 模板 codegen) · T-074 blocks→30 种 · T-077 微信素材 uploader(凭据隔离 OK)。orchestrator 集中接线：T-077 跨包边界(relay index 导出 wechat-asset 类型 + job-worker 改 `@wechat-flow/relay` 包名 import) + mcp-server 显式加 hono + http-sse TokenResolver scope 收窄。code-review：T-047/T-077 reviewer 派发本会话连接中断→orchestrator inline 复核 **approved_with_notes**(报告落盘 docs/reviews/code/CODE-REVIEW-T-047/T-077-r1.md)；T-073/T-074 非 security 延迟 sprint-review。生产挂载延后见待办「Sprint 5 wiring backlog」。**⚠ T-080 HELD（上游缺口）**：依赖 M-005 token 注册表(空表+shape 不符)，PRD F-003 AC-004 ≥60 token 五类目录未 seed → 已 tech-lead 定义 **T-123**（M-005 重塑 + token 目录 seed，先 T-123 再解封 T-080）；`auth.test`/`server.test` 暂用 `list_tokens` 作哨兵。下一候选入口：T-103 DESIGN / T-123 / wiring backlog。Sprint 5 全任务卡 = T-103(DESIGN) + T-043..T-049、T-116、T-117、T-051、T-055、T-073、T-074、T-075、T-077..T-084、T-112（见 [`dev-plan-wechat-flow-s5.md`](docs/dev-plan/dev-plan-wechat-flow-s5.md)）。人工检查点=[pre_dev]（已过），post_sprint 非门禁。Sprint 5 backlog 并入承接（SR-R2-002/003/005/006/007）。framework-feedback 已上报 [#374](https://github.com/lync-cyber/CataForge/issues/374)·[#375](https://github.com/lync-cyber/CataForge/issues/375)·[#376](https://github.com/lync-cyber/CataForge/issues/376)。
