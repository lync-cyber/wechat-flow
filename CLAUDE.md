...
- 当前阶段: development
- 上次完成: **T-104 深色主题 token 草稿 + 高密度 DiagnosticsPanel 稿（[PR #64](https://github.com/lync-cyber/wechat-flow/pull/64)）** —— 设计门禁链恢复后首卡（Penpot `localhost:9001` 已回 200、单实例修复后 MCP 授权）。产出 [`docs/design/dark-theme-preview.png`](docs/design/dark-theme-preview.png)（600×1439）+ [`docs/design/tokens/dark-theme-draft.json`](docs/design/tokens/dark-theme-draft.json)（34 深色 token，内嵌 WCAG 审计），绘于 Penpot 'Design System' 页 board 'T-104 深色主题预览'。**AC 全绿**：22 条诊断项（8 严重/8 提醒/6 提示，含 3 夜间风险）无截断可扫描（AC-001）、脚本**客观校验** WCAG 2.1 AA 16/16 文本前景 ≥4.5:1（最低 5.27）+ non-text 4/4 ≥3.0（装饰性 border 按 1.4.11 豁免）（AC-002）、全 `--color-{role}-dark` 命名（AC-003）、frame 导出（AC-004）。调性：暖炭底 `#1C1917` + 提亮墨绿 `#5AA491` + 陶红 `#E0796C`，延续浅色暖白+墨绿基调。用户视觉 sign-off 通过（EVENT-LOG `user_decision` ref=T-104）。**跨界经验**：Penpot MCP 运行于 POSIX 容器（拒 Windows 路径、fs 与宿主隔离），`export_shape` filePath 不能直写宿主仓——PNG 走 `execute_code` 取 bytes→base64 超限自动落宿主 `tool-results/*.txt`→Bash 解码校验，绕过隔离。前批 **T-128 五主题内容元素选择器样式增强已合（[PR #63](https://github.com/lync-cyber/wechat-flow/pull/63)）** —— 为 default/magazine/literary/business/tech 五套主题补全 `ul/ol/li/img/table/a` 的 `theme.blocks` 声明，绑定各主题自身 token（`ul/ol` margin-bottom=`--spacing-list-item` + list-style-position=`--align-list-marker`、`li` line-height=`--font-line-height-body`、`a` color=`--color-link` + text-decoration=`--decoration-link-underline`、`img` max-width:100%、`table` border-collapse:collapse），字面量值（非 `var()`，微信不支持）与 token 逐字一致（AC-004 强制）。**执行**：standard TDD（跨 5 包）—— RED（`tests/theme-content-elements.test.ts` 85 用例）→ GREEN（15 文件：5×list.ts/media.ts + 5×index.ts spread）→ REFACTOR skip（per-theme 数据结构同 heading.ts 既有范式，非 duplication 缺陷）。**AC-005 上游漂移纠偏**（upstream-caused，登记 §漂移聚合）：卡内 `renderToHtml` 命名漂移→实为 `renderMarkdown`；且 `<ul>` 被既有 T-015 `transform-list-to-table` 无条件转 `<table>`（微信兼容既定行为），故 AC-005 产物断言改测存活元素 ol/li/img/table/a + 新增 ul→table 显式验证（实证：ordered markdown 下 ol/li/img/table/a 均带正确 style，unordered 下 ul→table 承接 table 样式），ul 声明层由 AC-002/003 覆盖。**主线程独立复核**：T-128 测试 85/85；**全量回归首轮 5 失败=T-057 e2e golden 快照**（03/04/08/10/11 fixture 含 list/table，渲染合法变更），`UPDATE_SNAPSHOTS=1` 重生后 word-diff **逐字审计确认仅 style 增量**（`<ol style=...>`/`<li style=...>`/`<table style="border-collapse: collapse">`，无内容变异），全量复跑 **194 files / 2363 passed / 0 failed**；biome 16 文件 + tests-tsconfig + themes typecheck（--force 绕缓存 15/15）均 clean。**非设计门禁 backlog 至此清空**。前批 T-064 wire-in 已合（[PR #62](https://github.com/lync-cyber/wechat-flow/pull/62)）。
- 下一步行动: **设计门禁链已恢复并推进中**（Penpot infra 回归：`localhost:9001`=200、单实例修复后 MCP round-trip 正常）。链条 `T-104→T-130→T-131→T-113`，T-104 已 sign-off 合入。**下一卡 = T-130 设计一致性工具链**（feature，standard TDD，~420 LOC，deps 含 T-104✓/T-095..T-106✓）——构建 4 个 design-sync 脚本（`export-penpot-tokens` / `export-penpot-frames` / `token-diff` / `build-overlay-report`）+ Playwright `design-overlay` spec + 单测 + fixtures；固化 [`docs/design/tokens/penpot-tokens.json`](docs/design/tokens/penpot-tokens.json)（≥60 token）+ 23 UC frame + 5 P frame + `overlay-report.html`。**跨界约束（T-104 已验证）**：Penpot MCP 在 POSIX 容器（fs 隔离），token/frame 导出经 `execute_code` 取 bytes→超限落宿主 `tool-results/*.txt`→Bash 解码，`export-penpot-*.ts` 脚本须适配此路径而非直写 filePath。之后 **T-131**（视觉门禁 + 逐节 sign-off，残差转 `conditional_release.blocking_conditions`）→ **T-113**（Sprint 6 验证门，14 项）→ sprint-review 收尾。节奏：单卡一 PR、串行合并（§项目状态 单写区）。设计轨道 T-095..T-106 全 sign-off（含 T-104）。人工检查点 = [pre_dev]（已过），post_sprint 非门禁。**遗留（登记 sprint-review §wiring-completeness）**：① T-087/T-088 内置主题 `assets`/`paintable` 激活填充延迟（配套 Linux 视觉基线重播）；② T-085 skill bundle description 自动 optimizer 未跑（无头 `claude -p` 401）；③ **S6 deliverable/落点漂移聚合**（sprint-review 汇总 drift-rate，勿逐卡回改；root_cause=upstream-caused）；根因已打包上游反馈 `docs/feedback/fb-dev-plan-path-drift-writeback-20260701.md`。**T-113 forward-drift**：AC 引用 `e2e/visual/snapshots/variant/` 与实际 `snapshots/{platform}/{testFilePath}/` 路径结构不符，T-113 需调和。Sprint 6 任务卡见 [`dev-plan-wechat-flow-s6.md`](docs/dev-plan/dev-plan-wechat-flow-s6.md)。
