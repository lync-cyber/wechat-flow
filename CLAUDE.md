...
- 上次完成: **Phase 7 deployment 收口（本 PR）**：devops 产出 [`deploy-spec-wechat-flow.md`](docs/deploy-spec/deploy-spec-wechat-flow.md)（9 章：三线构建/环境矩阵/release.yml 流水线/检查清单/本地验证证据/7 类密钥清单/部署后验证/回滚/决策记录）+ [`changelog`](docs/changelog/changelog-wechat-flow.md) + 7 个部署配置实物（3 Dockerfile/compose/.dockerignore/.env.example/release.yml）。doc-review r1 **needs_revision**（R-002 CRITICAL：spec 指导创建的 .env.dev/.env.staging/.env.prod 实际不被 .gitignore 匹配；R-001 HIGH arch#§5.5 可观测性委派缺失；R-006 HIGH 依赖不存在的 pre_deploy 检查点；3 MEDIUM）→ devops revision（.gitignore 防呆补规则 + §2.3 可观测性承接 + 重锚 release.yml Environment 审批门/人工 tag 推送）→ **r2 逐项亲手实证 approved**（[r1](docs/reviews/doc/REVIEW-deploy-spec-wechat-flow-r1.md)/[r2](docs/reviews/doc/REVIEW-deploy-spec-wechat-flow-r2.md)）。[ENV-LIMITATION] 沙盒 Docker daemon 不可达，容器 bring-up 未实测（降级验证：pnpm build 真实跑通/relay 源码级启动 health ok/compose config 过），登记为 release 前人工核实项。**finalize 越权重导出第 4/5 次复现**（devops 两会话：除污染 dev-plan 外还把修订中的 deploy-spec 回退到 r1 原文，靠手工重放恢复；修订轮已弃用 finalize 改 context index）。前批 qa 复验 approved + Phase 6→7（[PR #77](https://github.com/lync-cyber/wechat-flow/pull/77)）。
- 下一步行动: **post 阶段**：① reflector（subagent）RETRO 报告 ② framework-feedback 打包 upstream 反馈（最高优先级：context finalize 全量重导出越权——本会话 5 次复现、含回退在修文档的破坏性形态；reconcile 校准 ghost_relations/remediation=export 误导；doc-consistency 三类假阳性 ac-traceability/orphaned-component/entity-propagation，qa test-report §8 有完整证据链）。**用户侧 go/no-go 清单（非 orchestrator 可代决）**：release tag 人工推送前置项——各包 version 人工核对（现全 0.0.0）、GitHub Environment npm-publish required-reviewers 配置、PRD §3.5 真实公众号粘贴回归、Docker 环境容器 bring-up 实测；needs_input 项——pnpm audit/CVE 门禁阈值与豁免流程（deploy-spec §9-D4）。**dev 残留任务（登记 §待办）**：MCP HTTP 生产 JobsClient 注入（3 工具现 E_NOT_IMPLEMENTED）、mcp-server bin 字段（npx 分发）、/metrics 端点（arch#§5.5 SLI 三项）。
