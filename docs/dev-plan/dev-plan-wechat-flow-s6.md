...
- **acceptance_criteria**:
  - [ ] AC-001: `tests/perf/cold-start.ts` 经 `runBenchmark` 测量 `createServer()` → `server.connect()`（ready）的冷启动耗时，每轮 `resetThemeRegistry()` 强制 `registerBuiltins` 做满工
  - [ ] AC-002: cold-start P95 阈值 800ms 经 `perf-runner-cli.ts` 强制（env `PERF_COLDSTART_P95_MS` 可覆盖，CI 可放宽）；套件内 `cold-start.test.ts` 验机制（samples / 单调百分位 / env 字段），与既有 typing-latency / theme-switch 约定一致
  - [ ] AC-003: ~~`startup.ts` 中 Playwright / BullMQ 惰性 `import()`~~ **N/A（前提失效）** → 重述为架构守卫不变量：`tests/mcp-server/cold-start-guard.test.ts` 断言 mcp-server 包 deps + src import 闭包**无** `playwright` / `bullmq` 静态依赖
  - [ ] AC-004: ~~`SKIP_PLAYWRIGHT=1` 单测~~ **N/A（前提失效）** → 由 AC-003 架构守卫替代
  - [ ] AC-005（新增，扩展范围）: `packages/core/src/pipeline/{parse,transform,serialize}.ts` 的 `unified()` processor 预编译为模块级 `freeze()` 单例；确定性回归测试断言重复调用 byte-identical
- **deliverables**:
  - [ ] `packages/core/src/pipeline/parse.ts` / `transform.ts` / `serialize.ts` — `unified()...freeze()` 模块级单例
  - [ ] `tests/perf/cold-start.ts` — `measureColdStart`（复用 `tests/perf/perf-runner.ts`）
  - [ ] `tests/perf/perf-runner-cli.ts` — 注册 cold-start 测量 + 800ms 阈值
  - [ ] `tests/perf/cold-start.test.ts` — bench 机制验证（node 环境）
  - [ ] `tests/mcp-server/cold-start-guard.test.ts` — 架构守卫（冷路径无 playwright/bullmq 静态 import）
  - [ ] `tests/core/pipeline-determinism.test.ts` — 单例预编译确定性回归
- **context_load**:
  - arch#§2.M-009
