...
- **deliverables**:
  - [ ] `packages/core/src/guard/` — 9 维度守卫实现
  - [ ] `packages/core/src/guard/contrast.ts` — WCAG AA 对比度计算（基于 WCAG 2.1 相对亮度公式）
  - [ ] `packages/core/src/guard/index.ts` — 导出 `validateThemeGuard`
...
- **acceptance_criteria**:
  ...
  - [ ] AC-005: 集成测试：mock 23 个 Tool 端点，按 SKILL.md 描述的顺序串调一次，全链路无异常（含 `describe_template`）
- **deliverables**:
  - [ ] `skill/SKILL.md`
