...
- **acceptance_criteria**:
  - [ ] AC-001（token 导出固化）: Given Penpot 项目内 T-095 已建立的 `Token-{group}` 变量组（`Token-Surface`/`Token-Border`/`Token-Text`/`Token-Brand`/`Token-Accent`/`Token-Functional`/`Token-Diagnostics`/`Token-Typography`/`Token-Spacing`/`Token-Shadow`/`Token-Animation`）与 T-104 补齐的 PS-008 深色 token; When 运行导出脚本 `scripts/design-sync/export-penpot-tokens.ts`（经 Penpot MCP 只读查询变量组）; Then 产出 `docs/design/tokens/penpot-tokens.json`，顶层 key 为 token 名（如 `--color-surface`），value 为 Penpot 侧记录值字符串；文件非空且含 ≥ 60 条 token 条目（对齐 `packages/themes/default/src/tokens.ts` 当前 60 个 token 的规模基线）。
  - [ ] AC-002（frame 导出固化 — 组件）: Given Penpot 项目内 T-097/T-106 已绘制的 UC-001..UC-023 全部组件视觉稿; When 运行 `scripts/design-sync/export-penpot-frames.ts --scope=components`; Then 每个 UC-NNN 至少 1 张 PNG 导出到 `docs/design/frames/components/UC-NNN.png`（多状态变体的组件导出其 `default` 状态 frame），23 个组件全部产出对应文件，缺失任一即断言失败并列出缺失的 UC-NNN 清单。
  - [ ] AC-003（frame 导出固化 — 页面）: Given Penpot 项目内 T-096/T-102/T-103/T-105 已绘制的 P-001..P-005 全部页面视觉稿; When 运行 `scripts/design-sync/export-penpot-frames.ts --scope=pages`; Then 每个 P-NNN 导出桌面档 frame 到 `docs/design/frames/pages/P-NNN-desktop.png`，5 个页面全部产出对应文件，缺失任一即断言失败并列出缺失的 P-NNN 清单。
  - [ ] AC-004（token-diff 硬校验脚本）: Given `docs/design/tokens/penpot-tokens.json`（AC-001 产物）与 5 套内置主题 `packages/themes/{default,magazine,literary,business,tech}/src/tokens.ts` 的 `tokens` 导出对象; When 运行 `pnpm design:token-diff`（`scripts/design-sync/token-diff.ts`）; Then 对每个在 Penpot token JSON 中存在的 token key，比对其值与 default 主题 `tokens.ts` 对应 key 的字面值（大小写不敏感的十六进制颜色归一化后比较；px/rem 数值型逐字符串比较），全部一致时进程 exit code 0 且输出 `{ mismatches: [] }`；任一不一致时 exit code 1 并输出 `mismatches: [{ tokenKey, penpotValue, codeValue }]` 明细列表，不静默通过。
  - [ ] AC-005（token-diff 单元测试覆盖不一致路径）: Given 构造的 fixture（`tests/design-sync/fixtures/mismatched-tokens.json`，其中 `--color-brand` 字面值与 `packages/themes/default/src/tokens.ts` 故意不同）; When 单元测试调用 `runTokenDiff(fixturePath, defaultTheme.tokens)`; Then 返回值 `mismatches` 数组长度 ≥ 1 且包含 `tokenKey === '--color-brand'` 的条目；同时构造完全一致的 fixture 验证 `mismatches` 长度为 0（正反两路径均被测试覆盖）。
  ...
  - [ ] AC-007（并排差异报告生成）: Given AC-002/AC-003 的 Penpot frame 导出图与 AC-006 的前端实际截图均已就绪; When 运行 `scripts/design-sync/build-overlay-report.ts`; Then 产出 `docs/design/reports/overlay-report.html`，报告内每个 UC-NNN / P-NNN 各占一节，左栏嵌入 Penpot frame 图、右栏嵌入前端截图，标题含 `{id}: {name}`；报告顶部统计行显示 `{componentsTotal}/23 组件、{pagesTotal}/5 页面` 已生成并排对照；生成的 HTML 文件非空且可被浏览器直接打开渲染（无外部资源依赖，图片以文件相对路径内嵌）。
- **deliverables**:
  - [ ] `scripts/design-sync/export-penpot-tokens.ts` — Penpot MCP 变量组只读查询 + JSON 序列化
  - [ ] `scripts/design-sync/export-penpot-frames.ts` — 组件/页面 frame 导出（`--scope=components|pages` 参数）
  - [ ] `scripts/design-sync/token-diff.ts` — 导出 `runTokenDiff(penpotTokensPath, codeTokens)`，CLI 与可测函数分离
  - [ ] `scripts/design-sync/build-overlay-report.ts` — 并排差异报告生成器
  - [ ] `docs/design/tokens/penpot-tokens.json` — Penpot token 导出固化产物
  - [ ] `docs/design/frames/components/UC-001.png` .. `UC-023.png` — 23 个组件 frame 导出
  - [ ] `docs/design/frames/pages/P-001-desktop.png` .. `P-005-desktop.png` — 5 个页面 frame 导出
  - [ ] `e2e/visual/design-overlay.spec.ts` — 新 Playwright project，实际编辑器 SPA 页面/组件截图
  - [ ] `docs/design/reports/overlay-report.html` — 并排差异报告产物
  - [ ] `tests/design-sync/token-diff.test.ts` — AC-005 一致/不一致两路径单元测试
  - [ ] `tests/design-sync/fixtures/mismatched-tokens.json` / `tests/design-sync/fixtures/matched-tokens.json`
  - [ ] `package.json` 根级新增 `design:token-diff` / `design:export-tokens` / `design:export-frames` / `design:build-overlay-report` scripts
- **relates_to**: [M-005]
- **context_load**:
...
- **acceptance_criteria**:
  - [ ] AC-001（token 硬校验全绿）: 运行 `pnpm design:token-diff`，进程 exit code 为 0，输出 `mismatches: []`；任一 mismatch 存在即视为本卡失败，不得以 `[ASSUMPTION]` 或 `[ENV-LIMITATION]` 豁免（token 一致性是客观可脚本验证项，不允许人工酌情放行）。
  - [ ] AC-002（frame 导出完整性）: 检查 `docs/design/frames/components/` 目录含 23 个 `UC-NNN.png` 文件、`docs/design/frames/pages/` 目录含 5 个 `P-NNN-desktop.png` 文件，与 `docs/design/tokens/penpot-tokens.json` 均存在于 git 索引（`git ls-files docs/design/` 非空）。
  - [ ] AC-003（并排差异报告生成）: 打开 `docs/design/reports/overlay-report.html`，报告顶部统计行显示 `23/23 组件、5/5 页面` 已生成并排对照，逐节浏览全部 28 节内容均左右两栏图片均可加载（非损坏图片）。
  - [ ] AC-004（人工视觉 sign-off）: 用户逐节目视比对 overlay-report.html 中前端截图与 Penpot frame 导出图，对每一节标记「一致」或「存在差异」；全部标记「一致」时签字记录以 `event=user_decision` 写入 `docs/EVENT-LOG.jsonl`，`detail` 含 `design_signoff T-131: 前端 vs Penpot 全量一致性核验通过` 字样，`ref=T-131`。
  - [ ] AC-005（残差差异转 blocking_conditions）: Given AC-004 核验中存在被标记「存在差异」的 UC-NNN / P-NNN 条目; When 该情况发生; Then 本卡不产出 `approved` verdict，而是产出 `conditional_release`，`blocking_conditions` 逐条列出 `{ condition: "{id} 视觉差异待收敛", owner: "developer", detail: "{差异描述}" }`，每条差异有明确 owner；`blocking_conditions` 非空前不视为本卡通过，不推进 Sprint 6 DONE。
- **deliverables**:
  - [ ] `docs/EVENT-LOG.jsonl` — design_signoff 事件（T-131，`event=user_decision`）
  - [ ] 若存在残差差异：`docs/reviews/sprint/` 下 Sprint 6 sprint-review 报告中登记 `blocking_conditions` 清单（本卡不新建审查报告文件，残差差异随 sprint-review 收口）
- **relates_to**: [M-005]
- **context_load**:
  - ui-spec-wechat-flow-uc001-uc014#§2
  - ui-spec-wechat-flow-p001-p005#§3
...
- **acceptance_criteria**:
  - [ ] **设计一致性门禁通过**：T-131 token 硬校验（`pnpm design:token-diff`）exit code 0 且 `mismatches: []`；`docs/EVENT-LOG.jsonl` 含 `ref=T-131` 的 `design_signoff` 核验记录；若存在残差视觉差异，对应 `conditional_release.blocking_conditions` 已全部闭环为空列表
