...
- [ASSUMPTION] 云端功能（素材库上传、Headless 渲染）需要后端服务，部署形态由 architect 阶段规划。依赖服务端的功能范围包括 F-005（长图/封面 Headless 渲染 + 素材库上传中继）、F-006（多图床 relay，凭据中继）、F-013（MCP server / API 中继），architect 阶段需统一规划这些后端服务的部署形态与共享边界。
- [ASSUMPTION] 视觉一致性 ≤ 5% 验收规范：算法 `pixelmatch`，threshold 0.2，`includeAA: false`；视口 1280×800（desktop）+ 375×667（mobile）；5 篇典型样本固定在 `tests/visual/samples/{theme}/{1..5}.md`（5 主题 × 1 篇）；计算口径 `mismatched_pixels / total_pixels ≤ 0.05`。
