...
共享参数（与 §4 [ASSUMPTION] 一致）：算法 `pixelmatch` (threshold 0.2, includeAA: false)；视口 1280×800 (desktop) + 375×667 (mobile)；5 篇典型样本固定在 `tests/visual/samples/{theme}/{1..5}.md`（5 主题 × 1 篇）；计算口径 `mismatched_pixels / total_pixels ≤ 0.05`。
