...
| MCP 鉴权 | F-013 AC-004 | 两级 API key：**普通 user key**（`scope=user`，调用 22 个 Tool + Relay 业务端点）+ **admin key**（`scope=admin`，独立鉴权基线，仅可调 API-028..API-031 管理端点）；per-key 配额（rate-limit 中间件）；key 创建 / 列出 / 轮换 / 吊销暴露 admin API（见 arch-wechat-flow-api §3.5）；key 哈希存储（不存明文，仅创建时一次性返回明文） |
...
| F-013 | 程序化调用 (MCP/CLI/Skill) | P1 | M-009 (MCP server, 22 Tool) / M-011 (CLI) / M-008 (应用层 use case) / M-012 (schema 契约层) / `skill/` 目录 (Skill bundle，编排说明与 prompts) |
...
| Q3.9 | CSS 内联化库锁定 `juice` 11.x（纯 JavaScript、跨四运行时无 Wasm 加载差异、`options.preserveImportant` / `removeStyleTags` 等开关确定性可控）；不切换 `css-inline` (Wasm) 以避免 Edge/Worker 加载漂移；CI 黄金 fixture (`tests/golden/inline-styled/`) 5 篇样本双向 SHA-256 守护 | §1.4 技术栈、M-002/M-008 内联化 stage、§7.4 CI 门禁 |
