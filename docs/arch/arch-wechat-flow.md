...
**MCP Tool 总数对账**：§3.1 共 23 个 Tool（**19 同步 + 4 异步**）= API-001..API-015（16 同步；其中 API-009 拆 `list_tokens` + `describe_token` 共 2 Tool）+ API-016 合并条目（4 异步长任务 `upload_image` / `upload_to_wechat_asset` / `export_long_image` / `export_cover` + 2 同步轮询入口 `get_job` + `get_ruleset_version`）+ API-033 `describe_template`（1 同步）。与 PRD §1.2.2 交付形态表的「MCP server 覆盖 23 个 Tool」字面一致。
