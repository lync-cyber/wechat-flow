...
- §3 接口契约 → 见分卷 arch-wechat-flow-api（API-001..API-016 含合并条目、API-017..API-031）
- §4 数据模型 → 见分卷 arch-wechat-flow-data（E-001..E-010）
...
补充说明：
- 编辑器与后端通过 Hono RPC 类型契约连接，凭据不进浏览器
...
| CSS 内联化 | juice / css-inline | 11 / 0.15 | Active | F-004 AC-003 CSS 内联化候选库，具体由 implementer 阶段择优 | juice / css-inline docs |
...
| 同步协议 | Yjs + y-codemirror.next + y-websocket | yjs 13.6.x / y-codemirror.next 0.3.x / y-websocket 服务端进程 | Active | F-012 CRDT 多人协作；编辑器内置 `y-codemirror.next` 接 CodeMirror 6 SourcePane；awareness 经 Redis pub/sub 中继；服务端快照写入 SQLite/Postgres（M-013） | yjs / y-codemirror.next docs + [rn-004](../research/tech-eval-stack-sync-protocol.md) |
...
接口定义见分卷 [arch-wechat-flow-api](./arch-wechat-flow-api.md)，含 API-001..API-016（合并条目）+ API-017..API-031。本主卷 frontmatter `sections_in_volumes."3"` 指向该分卷。

## 4. 数据模型

数据模型见分卷 [arch-wechat-flow-data](./arch-wechat-flow-data.md)，含 E-001..E-010。本主卷 frontmatter `sections_in_volumes."4"` 指向该分卷。
...
| 主题守护 | PRD §3.3 — 8 维静态校验阻断发布 | 主题 CI 任务在 `pnpm --filter @wechat-flow/themes-* validate` 跑 8 维校验，任一不通过阻断 |
...
| 沙箱隔离 | F-010 AC-008 / PRD §3.2 | 第三方插件运行在 Web Worker；Comlink RPC 仅暴露白名单 API（`registerBlock` / `registerVariant` / `registerRule`）；**网络访问采用"仅事件通道"模型**：Worker 内禁用 `fetch` / `XMLHttpRequest` / `WebSocket` 全局对象（通过 Worker 启动时 monkey-patch 删除），插件如需访问网络必须调用 `plugin-api.requestResource(url, init)` → 主线程接收请求 → 主线程依据 pack manifest `permissions.network[]` 白名单做 ACL 检查 → 主线程代为 fetch → 通过 Comlink 把 `Response` 结构化克隆回沙箱；ACL 检查拒绝时抛 `E_PERMISSION_DENIED`，所有放行调用写入 §5.5 审计日志 |
| MCP 鉴权 | F-013 AC-004 | 两级 API key：**普通 user key**（`scope=user`，调用 16 个 Tool + Relay 业务端点）+ **admin key**（`scope=admin`，独立鉴权基线，仅可调 API-028..API-031 管理端点）；per-key 配额（rate-limit 中间件）；key 创建 / 列出 / 轮换 / 吊销暴露 admin API（见 arch-wechat-flow-api §3.5）；key 哈希存储（不存明文，仅创建时一次性返回明文） |
...
flowchart LR
    ...
    subgraph Container[容器化部署单元]
      ...
      YwsServer[y-websocket Server - Yjs awareness 中继]
    end
...
- **y-websocket Server**：Yjs 协同子服务，独立 Node 进程或与 Hono 同进程 integration（按部署规模择优）；接受 WebSocket 连接、维护 Y.Doc 内存副本、订阅 Redis awareness channel；周期性 snapshot 持久化到 SQLite/Postgres（节流：每 60s 或 100 ops）
- **持久化**：
...
- **跨运行时测试**: `tests/cross-runtime/` 在 `node` / `worker` / `edge-runtime`（miniflare）三 target 跑同一 SHA-256 一致性测试；浏览器主线程一致性由 Playwright e2e 验证
...
- **门禁**：规则集 fixture 100% 通过、主题 8 维守护通过、视觉回归 ≤ 5% 像素差异
...
| F-008 | 模板市场 | P1 | M-005 (主题与组件注册中心) / M-001 (编辑器 UI) |
| F-009 | 主题继承与品牌包 | P2 | M-005 (主题与组件注册中心) |
| F-010 | 开发者扩展与插件系统 | P1 | M-007 (插件沙箱与 plugin-api) / M-011 (CLI) / M-012 (schema 契约层) |
| F-011 | 质量保障 | P0/P1 | M-003 (过滤规则集引擎) / M-004 (粘贴过滤模拟器) / M-005 (主题守护) |
| F-012 | 协作与同步 | P2 | M-013 (浏览器端持久化与同步) / M-010 (中继服务 - 同步后端) |
| F-013 | 程序化调用 (MCP/CLI/Skill) | P1 | M-009 (MCP server) / M-011 (CLI) / M-008 (应用层 use case) / M-012 (schema 契约层) |
