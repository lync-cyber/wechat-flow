...
[NAV]
- §4 数据模型 → §4.1 实体关系, E-001 Document, E-002 Theme, E-003 Pack, E-004 RulesetVersion, E-005 Job, E-006 Credential, E-007 UploadRecord, E-008 DiagnosticReport, E-009 YDocSnapshot, E-010 ApiKey, §4.2 DDL 兼容性
[/NAV]
...
> - **打包实体**（E-002 Theme、E-003 Pack、E-004 RulesetVersion）以静态 manifest + 资产文件形态在 npm registry 分发，运行时仅在内存中索引，无需数据库存储
...
| frontmatter | object | required | 解析后的 frontmatter 对象（theme / paint / base-color / template / 自定义变量） |
...
| templates | array | optional，≥ 2 (内置主题强制) | 该主题随发的内容模板列表 |
...
| nodeDiffs | array | optional | 粘贴过滤模拟产出的逐节点差异（F-002 AC-006） |
| generatedAt | datetime | required | 生成时间 |

映射：F-002 AC-005 / AC-006 / F-007 AC-004 / F-011 AC-002
...
| quotaConfig | TEXT (JSON) | required | per-key 配额配置：`{ rpm: number, burstSize: number, monthlyJobCap: number }` |
| createdAt | TEXT (ISO datetime) | required | 创建时间 |
