...
4. **工具栏按钮组**（右对齐，从左到右顺序）：
   ...
   - 「**复制到公众号**」（C-003 **primary** variant，映射 F-004 主入口）：文字「复制」；**桌面档（≥1280px）**：80×32px 图标+文字；**平板/移动档（<1280px）**：32×32px 纯图标
...
**状态与交互**：
- 点击「兼容性摘要」触发 C-013 展开/折叠（与 `isExpanded` 双向同步）；该区域 hover 时背景 `--color-surface-hover`，focus（键盘聚焦）时显示 2px focus ring；其他指标区域无 hover 反馈
- 兼容性严重项 > 0 时显示 `--color-error`，提醒项 > 0 时显示 `--color-warning`，否则 `--color-text-muted`
- 可读性/违规词/夜间风险同样使用三态色规则（见主卷 A-006）

**平板简化**：
- 保留「字数 / 兼容性 / 可读性 / 夜间风险」四项
- 违规词在平板档降级为 `i` 图标 + 数字 tooltip（图标尺寸 16×16px），详情仍由 C-013 展示
